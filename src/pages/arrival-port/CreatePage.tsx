...
export default function ArrivalPortFormPage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("arrivalPort", "view");
  const canEdit = usePermission("arrivalPort", "edit");
  const canDelete = usePermission("arrivalPort", "delete");
  ...
  const [popoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/arrival-port/create");
        } else {
          navigate("/arrival-port/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Arrival port record created successfully!");
      handleReset();
    } else {
      toastSuccess("Arrival port record created successfully!");
      navigate("/arrival-port");
    }
  };
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Arrival Port" : "Creating Arrival Port"}
        videoSrc={video}
