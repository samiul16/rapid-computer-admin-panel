...
export default function AwardListsDetails() {
  const navigate = useNavigate();
  const { isRTL } = useSelector((state: RootState) => state.language);

  const [keepChanges, setKeepChanges] = useState(false);
  const [isOptionModalOpen, setIsOptionModalOpen] = useState(false);
  const [selectedLeaveType, setSelectedLeaveType] = useState("Bed 1");
  ...
  // get permission
  // const canCreate: boolean = usePermission("awardLists", "create");
  // const canView: boolean = usePermission("awardLists", "view");
  // const canEdit: boolean = usePermission("awardLists", "edit");
  // const canDelete: boolean = usePermission("awardLists", "delete");
  // const canExport: boolean = usePermission("awardLists", "export");
  const canPdf: boolean = usePermission("awardLists", "pdf");
  const canPrint: boolean = usePermission("awardLists", "print");
  const canSeeHistory: boolean = usePermission("awardLists", "history");
  ...
  const getLeavesData = (leaveType: string): TermsData => {
    const leavesMap: Record<string, TermsData> = {
      "Employee of the Month": initialData,
      "Best Team Player": initialData,
      "Best Employee": initialData,
    };
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[leavesData]}
          title="Award Lists Details"
          subtitle="Award Lists Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "award-lists-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Award Lists"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/award-lists")}
        listText="List"
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/award-lists/create"),
          },
          {
            label: "Edit",
            icon: <Edit className="w-5 h-5 text-blue-600" />,
            onClick: () => navigate("/award-lists/edit/1"),
          },
        ]}
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            {fieldPermissions.employeeName && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.employeeName)}
                </div>
              </div>
            )}
            {fieldPermissions.awardBy && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Award By</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.awardBy)}
                </div>
              </div>
            )}
            {fieldPermissions.date && (
              <div className="">
            ...
            {fieldPermissions.giftItem && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Gift Item</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.giftItem)}
                </div>
              </div>
            )}
            {fieldPermissions.awardDescription && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Award Description
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.awardDescription)}
                </div>
              </div>
            )}
