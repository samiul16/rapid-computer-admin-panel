...
export default function ComponentLevelGridView({
  ...
  const [gridData, setGridData] = useState(awardData);
  const canDelete: boolean = usePermission("awardLists", "delete");
  const canRestore: boolean = usePermission("awardLists", "restore");
  const canEdit: boolean = usePermission("awardLists", "edit");

  // Debug permissions
  console.log("awardLists Permissions:", {
    canDelete,
    canRestore,
    canEdit,
  });
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      ...
      awardBy: `Awarded by ${index + 1}`,
      date: `Date ${index + 1}`,
      giftItem: `Gift ${index + 1}`,
      awardDescription: `Description ${index + 1}`,
  ...
  const filteredData = gridData.filter(
    (item) =>
      ...
      item.awardBy.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.date.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.giftItem.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.awardDescription.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Total {gridData.length} Award Lists
        </span>
      </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 items-center gap-2 mb-4">
                  ...
                  <CardTitle
                    ...
                    onClick={() => navigate(`/award-lists/1`)}
                  >
                    {item.awardName}
                  </CardTitle>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (canRestore && item.isDeleted) {
                            ...
                            toastRestore("Award restored successfully");
                          } else {
                            if (canDelete) {
                              handleDeleteClick(item.id);
                              toastDelete("Award deleted successfully");
                            }
                          }
                        }}
                      >
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">Loading more award lists...</span>
              </div>
            </div>
          )}
