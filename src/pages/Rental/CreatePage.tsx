...
export default function RentalFormPage({ isEdit = false }: Props) {
  ...
  const canCreate: boolean = usePermission("rental", "create");
  const canView: boolean = usePermission("rental", "view");
  ...
  const name: boolean = usePermission("rental", "create", "name");
  const description: boolean = usePermission("rental", "create", "description");
  const isDefault: boolean = usePermission("rental", "create", "isDefault");
  const isDraft: boolean = usePermission("rental", "create", "isDraft");
  const canPdf: boolean = usePermission("rental", "pdf");
  const canPrint: boolean = usePermission("rental", "print");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Rental created successfully!");
      handleReset();
    } else {
      toastSuccess("Rental created successfully!");
      navigate("/rental");
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("Rental on pdf click", formData);
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? labels.creatingRental : labels.creatingRental}
        videoSrc={video}
