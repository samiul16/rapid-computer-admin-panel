...
export default function AuditsCreatePage({ isEdit = false }: Props) {
  ...
  // Permission checks
  const canCreate = usePermission("audits", "create");
  const canView = usePermission("audits", "view");
  const canEdit = usePermission("audits", "edit");
  const canDelete = usePermission("audits", "delete");
  const canPdf: boolean = usePermission("audits", "pdf");
  const canPrint: boolean = usePermission("audits", "print");
  ...
  const permissionsFieldLevel = usePermission<keyof ProjectTypeDataType>(
    "audits",
    "create",
    ["title", "auditor", "auditDate", "status", "isDefault", "isDraft"]
  );
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          ...
          navigate("/audits/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Audit created successfully!");
      handleReset();
    } else {
      toastSuccess("Audit created successfully!");
      navigate("/audits");
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Audit saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Audit" : "Creating Audit"}
        videoSrc={video}
        ...
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              ...
              {permissionsFieldLevel.auditor && (
                <div className="space-y-2 relative">
                  <Autocomplete
                    ref={(el: any) => setRef("auditor")(el)}
                    id="auditor"
                    name="auditor"
                    options={["Almost John", "Maybe John", "Not John"]}
              ...
              {permissionsFieldLevel.auditDate && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("auditDate")}
                    type="date"
                    id="auditDate"
                    name="auditDate"
                    ...
                    onCancel={() => setFormData({ ...formData, auditDate: "" })}
                    labelText="Audit Date"
                    tooltipText="Enter audit date"
                    required
                  />
                </div>
              )}

              {permissionsFieldLevel.status && (
                <div className="space-y-2 relative">
                  <Autocomplete
                    ref={(el: any) => setRef("status")(el)}
                    id="status"
                    name="status"
                    options={["Completed", "Pending", "Not Started"]}
