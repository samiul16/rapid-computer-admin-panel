...
export default function AuditsDetails() {
  ...
  const [selectedLeaveType, setSelectedLeaveType] = useState("Financial Audit");
  ...
  // get permission
  // const canCreate: boolean = usePermission("audits", "create");
  // const canView: boolean = usePermission("audits", "view");
  // const canEdit: boolean = usePermission("audits", "edit");
  // const canDelete: boolean = usePermission("audits", "delete");
  // const canExport: boolean = usePermission("audits", "export");
  const canPdf: boolean = usePermission("audits", "pdf");
  const canPrint: boolean = usePermission("audits", "print");
  const canSeeHistory: boolean = usePermission("audits", "history");

  // Field-level permissions

  const fieldPermissions = usePermission<keyof TermsData>("audits", "view", [
    "title",
    "auditor",
    "auditDate",
    "status",
  ...
  const getLeavesData = (leaveType: string): TermsData => {
    const leavesMap: Record<string, TermsData> = {
      "Financial Audit": initialData,
      "Just Audit": initialData,
      "Feels Like Audit": initialData,
    };
  ...
  return (
    <>
      <PageLayout
        title="Viewing Audits"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/audits")}
        listText="List"
        listPath="audits"
        activePage="view"
        popoverOptions={[
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            {fieldPermissions.auditDate && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Audit Date</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.auditDate)}
                </div>
              </div>
            )}
            {fieldPermissions.auditor && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Auditor</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.auditor)}
                </div>
              </div>
            )}
            {fieldPermissions.status && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.status)}
                </div>
              </div>
            )}
