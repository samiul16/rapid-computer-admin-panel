...
export default function JobCategoriesCreatePage({ isEdit = false }: Props) {
  ...
  const name: boolean = usePermission("jobCategories", "create", "name");
  ...
  const isDraft: boolean = usePermission("jobCategories", "create", "isDraft");
  ...
  const permissionsFields = usePermission<keyof MockData>(
    "jobCategories",
    "create",
    ["name", "description", "startDate", "endDate", "status", "isDefault"]
  );
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/job-categories/create");
        } else {
          navigate("/job-categories/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Job Categories created successfully!");
      handleReset();
    } else {
      toastSuccess("Job Categories created successfully!");
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[formData]}
          title="Job Categories Details"
          subtitle="Job Categories Information"
        />
      ).toBlob();
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Job Categories saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={
          isEdit ? labels.editingJobCategories : labels.creatingJobCategories
        }
        videoSrc={video}
