...
export default function PatientDetails() {
  ...
  // Get patient data based on selected patient
  const getPatientData = (patientName: string): DetailsPageTypes => {
    const patientMap: Record<string, DetailsPageTypes> = {
      "John Doe": initialData,
      "Jane Smith": initialData,
      "Ahmed Khan": initialData,
      "Liu Wen": initialData,
      "Carlos Diaz": initialData,
      "Sarah Johnson": initialData,
      "Michael Brown": initialData,
      "Emily Davis": initialData,
      "David Wilson": initialData,
      "Lisa Anderson": initialData,
      "Robert Taylor": initialData,
      "Maria Garcia": initialData,
    };

    return patientMap[patientName] || initialData;
  };

  const [patientData, setPatientData] = useState<DetailsPageTypes>(
    getPatientData(selectedPatient)
  );
  ...
  useEffect(() => {
    setPatientData(getPatientData(selectedPatient));
  }, [selectedPatient]);
  ...
  const handlePrintPatient = (patientData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", patientData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintPatient(patientData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Patient Selection, Patient Details */}
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((patientData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(patientData.createdAt),
          updated: getRelativeTime(patientData.updatedAt),
          drafted: getRelativeTime(patientData.draftedAt),
          deleted: getRelativeTime(patientData.deletedAt),
        }}
      />
