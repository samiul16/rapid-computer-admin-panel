...
export default function PayslipDetails() {
  ...
  // Get payslip data based on selected employee
  const getPayslipData = (employeeName: string): DetailsPageTypes => {
    const payslipMap: Record<string, DetailsPageTypes> = {
      "Ahmed Al-Rashid": initialData,
      "Fatima Hassan": initialData,
      "Mohammed Ali": initialData,
      "Aisha Khan": initialData,
      "Omar Abdullah": initialData,
      "Layla Ibrahim": initialData,
    };

    return payslipMap[employeeName] || initialData;
  };

  const [payslipData, setPayslipData] = useState<DetailsPageTypes>(
    getPayslipData(selectedEmployee)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update payslip data when selection changes
  useEffect(() => {
    setPayslipData(getPayslipData(selectedEmployee));
  }, [selectedEmployee]);
  ...
  const handlePrintPayslip = (payslipData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", payslipData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintPayslip(payslipData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Employee Selection, Payslip Details */}
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((payslipData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(payslipData.createdAt),
          updated: getRelativeTime(payslipData.updatedAt),
          drafted: getRelativeTime(payslipData.draftedAt),
          deleted: getRelativeTime(payslipData.deletedAt),
        }}
      />
