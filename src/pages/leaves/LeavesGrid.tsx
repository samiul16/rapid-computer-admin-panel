...
export default function LeavesGrid({
  ...
  const canRestore: boolean = usePermission("leaves", "restore");
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      leaveTypes: leaveTypes[Math.floor(Math.random() * leaveTypes.length)],
      notes: "Sample leave note",
      isDefault: false,
  ...
  const filteredLeaves = leavesDataState.filter(
    (leave) =>
      leave.leaveTypes.toLowerCase().includes(searchQuery.toLowerCase()) ||
