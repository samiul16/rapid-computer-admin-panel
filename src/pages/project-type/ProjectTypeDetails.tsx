...
export default function ProjectTypeDetailsPage() {
  ...
  // get permission
  const canCreate: boolean = usePermission("projectTypes", "create");
  const canView: boolean = usePermission("projectTypes", "view");
  const canEdit: boolean = usePermission("projectTypes", "edit");
  const canDelete: boolean = usePermission("projectTypes", "delete");
  const canExport: boolean = usePermission("projectTypes", "export");
  const canPdf: boolean = usePermission("projectTypes", "pdf");
  const canPrint: boolean = usePermission("projectTypes", "print");
  const canSeeHistory: boolean = usePermission("projectTypes", "history");
  ...
  const canViewCode: boolean = usePermission("projectTypes", "view", "code");
  ...
  const getLeavesData = (leaveType: string): ProjectTypeData => {
    const leavesMap: Record<string, ProjectTypeData> = {
      Internal: {
        projectType: "Internal",
        code: "PRJ-001",
        description: "Internal Project",
      ...
      "Client-Based": {
        ...
        code: "PRJ-002",
        description: "Client-Based Project",
        isDefault: true,
        isActive: true,
        isDraft: false,
  ...
  const [leavesData, setLeavesData] = useState<ProjectTypeData>(
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[leavesData]}
          title="Project Type Details"
          subtitle="Project Type Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "project-type-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Project Type"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/project-types")}
        listText="List"
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/project-types/create"),
          },
          {
            label: "Edit",
