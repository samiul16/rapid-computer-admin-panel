...
export default function AssetsMasterDetailsPage() {
  ...
  const canCreate: boolean = usePermission("assetsMaster", "create");
  const canView: boolean = usePermission("assetsMaster", "view");
  const canEdit: boolean = usePermission("assetsMaster", "edit");
  const canDelete: boolean = usePermission("assetsMaster", "delete");
  ...
  // get permission
  const canCreate2: boolean = usePermission("assetsMaster", "create");
  const canEdit2: boolean = usePermission("assetsMaster", "edit");
  const canDelete2: boolean = usePermission("assetsMaster", "delete");
  const canExport2: boolean = usePermission("assetsMaster", "export");
  const canPdf: boolean = usePermission("assetsMaster", "pdf");
  const canPrint: boolean = usePermission("assetsMaster", "print");
  const canSeeHistory: boolean = usePermission("assetsMaster", "history");
  ...
  useEffect(() => {
    ...
    if (isViewPage) {
      assetsData = {
        ...
        category: "Building",
        assetName: "United States",
        description: "The most populous Assets in the world",
        status: "active",
        isDefault: true,
        isActive: true,
        isDraft: false,
  ...
  const handlePrintAssets = (assetsData: any) => {
    try {
  ...
  const handleSwitchChange = (checked: boolean) => {
    ...
    // if (checked) {
    //   setTimeout(() => handlePrintAssets(assetsData), 100);
    // }
  };
  ...
  return (
    <>
      <PageLayout
        title={t("button.viewingAssetsMaster")}
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/assets-master")}
        listText="List"
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/assets-master/create"),
          },
          {
            label: "Edit",
            icon: <Edit className="w-5 h-5 text-blue-600" />,
            onClick: () => navigate("/assets-master/edit/1"),
          },
        ]}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
          ...
          <div className="">
            ...
            <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
              {displayValue(assetsData.category)}
            </div>
          </div>
