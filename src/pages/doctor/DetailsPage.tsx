...
export default function DoctorDetails() {
  ...
  // Get doctor data based on selected doctor
  const getDoctorData = (doctorName: string): DetailsPageTypes => {
    const doctorMap: Record<string, DetailsPageTypes> = {
    ...
    return doctorMap[doctorName] || initialData;
  };

  const [doctorData, setDoctorData] = useState<DetailsPageTypes>(
    getDoctorData(selectedDoctor)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update doctor data when selection changes
  useEffect(() => {
    setDoctorData(getDoctorData(selectedDoctor));
  }, [selectedDoctor]);
  ...
  const handlePrintDoctor = (doctorData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", doctorData);
    ...
    <>
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Doctor Selection, Doctor Details */}
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((doctorData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(doctorData.createdAt),
          updated: getRelativeTime(doctorData.updatedAt),
          drafted: getRelativeTime(doctorData.draftedAt),
          deleted: getRelativeTime(doctorData.deletedAt),
        }}
      />
