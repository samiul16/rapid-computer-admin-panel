...
export default function ContractDetails() {
  ...
  // Get project group data based on selected project group
  const getProjectGroupData = (projectGroupName: string): DetailsPageTypes => {
    const projectGroupMap: Record<string, DetailsPageTypes> = {
      "TechCorp Solutions": initialData,
      "DataFlow Inc": initialData,
      "RetailMax Ltd": initialData,
      "SecureBank Corp": initialData,
      "Enterprise Systems": initialData,
      "PaymentGateway LLC": initialData,
      "Creative Agency Pro": initialData,
      "SpeedTech Solutions": initialData,
      "QualityAssurance Inc": initialData,
      "DocuTech Systems": initialData,
      "SupportTech Corp": initialData,
      "Enterprise Client": initialData,
    };

    return projectGroupMap[projectGroupName] || initialData;
  };

  const [projectGroupData, setProjectGroupData] = useState<DetailsPageTypes>(
    getProjectGroupData(selectedContract)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update project group data when selection changes
  useEffect(() => {
    setProjectGroupData(getProjectGroupData(selectedContract));
  }, [selectedContract]);
  ...
  const handlePrintProjectGroup = (projectGroupData: any) => {
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          ...
          isDefault: "Default Project Group",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", projectGroupData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintProjectGroup(projectGroupData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Contract Selection, Contract Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.projectGroup && (
              <div className="mt-1">
                <Autocomplete
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((projectGroupData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(projectGroupData.createdAt),
          updated: getRelativeTime(projectGroupData.updatedAt),
          drafted: getRelativeTime(projectGroupData.draftedAt),
          deleted: getRelativeTime(projectGroupData.deletedAt),
        }}
      />
