...
export default function ExpensesGrid({
  ...
  const canRestore: boolean = usePermission("realestateAgent", "restore");
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      fastName: `Name ${Date.now()}-${index}`,
      expense: `Expense ${Date.now()}-${index}`,
      hourlyRate: `${Math.floor(Math.random() * 1000)}`,
      company: `Company ${Date.now()}-${index}`,
  ...
  const filteredData = gridData.filter(
    (item) =>
      item.expense.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Total {gridData.length} Real Estate Agents
        </span>
      </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 items-center gap-2 mb-4">
                  ...
                  <CardTitle
                    ...
                    onClick={() => navigate(`/realestate-agent/1`)}
                  >
                    {item.fastName}
                  </CardTitle>
