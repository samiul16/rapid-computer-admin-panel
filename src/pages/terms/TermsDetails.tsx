...
export default function TermsDetailsPage() {
  ...
  // get permission
  const canCreate: boolean = usePermission("terms", "create");
  const canView: boolean = usePermission("terms", "view");
  const canEdit: boolean = usePermission("terms", "edit");
  const canDelete: boolean = usePermission("terms", "delete");
  const canExport: boolean = usePermission("terms", "export");
  const canPdf: boolean = usePermission("terms", "pdf");
  const canPrint: boolean = usePermission("terms", "print");
  const canSeeHistory: boolean = usePermission("terms", "history");

  // Field-level permissions
  const canViewTermsConditions: boolean = usePermission(
    "terms",
    "view",
    "termsConditions"
  );
  const canViewCode: boolean = usePermission("terms", "view", "code");
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[leavesData]}
          title="Terms & Conditions Details"
          subtitle="Terms & Conditions Information"
        />
      ).toBlob();
  ...
  return (
    <>
      <PageLayout
        title="Viewing Terms & Conditions"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/terms")}
        listText="List"
        listPath="terms"
        activePage="view"
        popoverOptions={[
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {canViewTermsConditions && (
              <div className="mt-1">
                <Autocomplete
                  ...
                  placeholder=" "
                  displayKey="termsConditions"
                  valueKey="termsConditions"
                  searchKey="termsConditions"
                  disabled={false}
            ...
            {canViewTermsConditions && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.termsConditions)}
                </div>
              </div>
            )}
          </div>
