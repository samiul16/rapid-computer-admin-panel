...
export default function CategoryCreatePage({ isEdit = false }: Props) {
  ...
  useEffect(() => {
    ...
    if (shouldAutoRestore) {
      // moduleData IS the saved form data, no need for .formData
      const savedFormData = moduleData;
      ...
      const scrollPosition = getModuleScrollPosition("category-form-module");
      if (scrollPosition) {
        setTimeout(() => {
  ...
  const handleReset = async () => {
    ...
    if (hasMinimizedData) {
      try {
        await resetModuleData("category-form-module");
  ...
  const handleMinimize = useCallback((): CategoryFormData => {
    return {
      ...formData,
    };
  }, [formData]);
    ...
    <>
              ...
              {permissionsFieldLevel.isDefault && (
                <div className="space-y-2 relative">
                  <SwitchSelect
                    ...
                    className="relative"
                    tooltipText="Set as default category"
                  />
                </div>
              )}

              {/* Status field - Moved to same row */}
      ...
      >
        <TemplateContent
          ...
          data={[
            { name: "Electronics", group: "Technology" },
            { name: "Clothing", group: "Fashion" },
            { name: "Books", group: "Education" },
            { name: "Home Decor", group: "Interior" },
            { name: "Sports", group: "Fitness" },
            { name: "Beauty", group: "Personal Care" },
          ]}
