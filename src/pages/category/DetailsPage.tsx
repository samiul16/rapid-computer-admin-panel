...
export default function CategoryDetails() {
  ...
  // Get category data based on selected category
  const getCategoryData = (categoryName: string): DetailsPageTypes => {
    const categoryMap: Record<string, DetailsPageTypes> = {
      Electronics: {
        ...initialData,
        name: "Electronics",
        group: "Technology",
    ...
    return categoryMap[categoryName] || initialData;
  };

  const [categoryData, setCategoryData] = useState<DetailsPageTypes>(
    getCategoryData(selectedSubCategory)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update category data when selection changes
  useEffect(() => {
    setCategoryData(getCategoryData(selectedSubCategory));
  }, [selectedSubCategory]);
  ...
  const handlePrintCategory = (categoryData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", categoryData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintCategory(categoryData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Category Selection, Category Details */}
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((categoryData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(categoryData.createdAt),
          updated: getRelativeTime(categoryData.updatedAt),
          drafted: getRelativeTime(categoryData.draftedAt),
          deleted: getRelativeTime(categoryData.deletedAt),
        }}
      />
