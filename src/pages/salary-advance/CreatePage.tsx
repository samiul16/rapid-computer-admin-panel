...
export default function LeavesFormPage({ isEdit = false }: Props) {
  ...
  // File upload states (removed for salary advance)

  // Permission checks
  const canCreate = usePermission("salaryAdvance", "create");
  const canView = usePermission("salaryAdvance", "view");
  const canEdit = usePermission("salaryAdvance", "edit");
  const canDelete = usePermission("salaryAdvance", "delete");
  ...
  const date: boolean = usePermission("salaryAdvance", "create", "date");
  const iqamaNo: boolean = usePermission("salaryAdvance", "create", "iqamaNo");
  const branch: boolean = usePermission("salaryAdvance", "create", "branch");
  const paymentMode: boolean = usePermission(
    "salaryAdvance",
    "create",
    "paymentMode"
  );
  const advanceAmount: boolean = usePermission(
    "salaryAdvance",
    "create",
    "advanceAmount"
  );
  ...
  const isDraft: boolean = usePermission("salaryAdvance", "create", "isDraft");
  ...
  // Form state
  const [formData, setFormData] = useState<SalaryAdvanceData>({
    date: "",
    iqamaNo: "",
    branch: "",
    paymentMode: "",
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/salary-advance/create");
        } else {
          navigate("/salary-advance/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Salary advance created successfully!");
      handleReset();
    } else {
      toastSuccess("Salary advance created successfully!");
  ...
  const handleReset = () => {
    ...
    // Reset file preview (removed for salary advance)
  ...
  // File upload handlers (removed for salary advance)
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Salary advance saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Salary Advance" : "Creating Salary Advance"}
        videoSrc={video}
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          >
              ...
              {advanceAmount && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    onCancel={() =>
                      setFormData({ ...formData, advanceAmount: 0 })
                    }
                    labelText="Advance Amount"
                    tooltipText="Enter advance amount"
                    required
                  />
                </div>
              )}
            ...
            {/* Third row - removed file upload for salary advance */}
          </form>
        </div>
      </PageLayout>
