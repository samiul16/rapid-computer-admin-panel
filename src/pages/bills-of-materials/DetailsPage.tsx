...
const projectTypeOptions = ["Laptop", "Mobile", "PC"];
...
export default function BillsOfMaterialsDetails() {
  const navigate = useNavigate();
  const { isRTL } = useSelector((state: RootState) => state.language);

  const [keepChanges, setKeepChanges] = useState(false);
  const [isOptionModalOpen, setIsOptionModalOpen] = useState(false);
  const [selectedLeaveType, setSelectedLeaveType] = useState("Laptop");
  ...
  // get permission
  // const canCreate: boolean = usePermission("billsOfMaterials", "create");
  // const canView: boolean = usePermission("billsOfMaterials", "view");
  // const canEdit: boolean = usePermission("billsOfMaterials", "edit");
  // const canDelete: boolean = usePermission("billsOfMaterials", "delete");
  // const canExport: boolean = usePermission("billsOfMaterials", "export");
  const canPdf: boolean = usePermission("billsOfMaterials", "pdf");
  const canPrint: boolean = usePermission("billsOfMaterials", "print");
  const canSeeHistory: boolean = usePermission("billsOfMaterials", "history");
  ...
  return (
    <>
      <PageLayout
        title="Viewing Bills of Materials"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/bills-of-materials")}
        listText="List"
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/bills-of-materials/create"),
          },
          {
            label: "Edit",
            icon: <Edit className="w-5 h-5 text-blue-600" />,
            onClick: () => navigate("/bills-of-materials/edit/1"),
          },
        ]}
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            {fieldPermissions.productVariant && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Product Variant
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.productVariant)}
                </div>
              </div>
            )}
            {fieldPermissions.quantity && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Quantity</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.quantity)}
                </div>
              </div>
            )}
            {fieldPermissions.unitOfMeasure && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Unit of Measure
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.unitOfMeasure)}
                </div>
              </div>
            )}
            {fieldPermissions.routing && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Routing</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.routing)}
                </div>
              </div>
            )}
            {fieldPermissions.bomType && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">BOM Type</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.bomType)}
                </div>
              </div>
            )}
            {fieldPermissions.manufacturingReadiness && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Manufacturing Readiness
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.manufacturingReadiness)}
                </div>
              </div>
            )}
            {fieldPermissions.consumption && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Consumption</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.consumption)}
                </div>
              </div>
            )}
