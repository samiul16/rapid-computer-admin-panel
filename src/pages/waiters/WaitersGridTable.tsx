...
export default function WaitersGridTable({
  setViewMode,
}: {
  ...
  const [waitersData, setWaitersData] = useState(mockWaiters);
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      code: `${(waitersData.length + index + 1)
        .toString()
        .padStart(2, "0")} Waiter`,
      name: `Waiter${waitersData.length + index + 1}`,
      photo: `/customer-dummy-image.jpg`,
      status:
        Math.random() > 0.3
          ? "active"
    ...
    if (waitersData.length >= 46) {
      setHasMore(false);
    } else {
      setWaitersData((prev) => [...prev, ...(newItems as Waiter[])]);
      setPage((prev) => prev + 1);
    }

    setIsLoading(false);
  }, [waitersData.length, isLoading, hasMore]);
    ...
    setWaitersData((prev) =>
      prev.map((waiter) =>
        waiter.id === id
          ? {
              ...waiter,
              isDeleted: waiter.isDeleted === true ? false : true,
            }
          : waiter
      )
    );
    ...
    setWaitersData((prev) =>
      prev.map((waiter) =>
        waiter.id === id
          ? {
              ...waiter,
              status: waiter.status === "active" ? "inactive" : "active",
            }
          : waiter
      )
    );
  ...
  const filteredWaiters = waitersData.filter(
    (waiter) =>
      waiter.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      waiter.code.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="px-4 py-3 h-full flex flex-col bg-white dark:bg-gray-900 parent">
            ...
            {filteredWaiters.map((waiter) => (
              ...
              >
                ...
                <div className="grid grid-cols-3 items-center gap-2 mb-4">
                  {/* Left - Title */}
                  <Tooltip label={waiter.name} position="top" withArrow>
                    <CardTitle
                      className="text-lg font-semibold cursor-pointer hover:text-primary transition-colors truncate"
                      onClick={() => navigate(`/waiters/1`)}
                    >
                      {waiter.name}
                    </CardTitle>
                  </Tooltip>
                  ...
                  <div className="flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    >
                      <div
                        ...
                        onClick={() => {
                          toggleStatus(waiter.id);
                          toastSuccess(
                            waiter.status === "active"
                              ? "Waiter activated successfully"
                              : "Waiter inactivated successfully"
                          );
                        }}
                      >
                        {waiter.status === "active" ? (
                    ...
                    {/* Delete/Restore */}
                    <Tooltip
                      label={waiter.isDeleted ? "Restore" : "Delete"}
                      position="top"
                      withArrow
                    >
                      <div
                        className={`cursor-pointer p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${
                          waiter.isDeleted ? "text-blue-500" : "text-red-500"
                        }`}
                        onClick={() => {
                          if (waiter.isDeleted) {
                            handleRestoreClick(waiter.id);
                          } else {
                            handleDeleteClick(waiter.id);
                          }
                          toastSuccess(
                            waiter.isDeleted
                              ? "Waiter restored successfully"
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">Loading more waiters...</span>
              </div>
            </div>
          )}

          {/* End of data indicator */}
          {!hasMore && filteredWaiters.length > 12 && (
            <div className="flex justify-center items-center py-8">
              <span className="text-sm text-gray-500 dark:text-gray-400">
                No more waiters to load
              </span>
            </div>
          )}
        </div>
        ...
        {isFilterOpen && (
          <div className="w-80 flex-shrink-0 border-l border-gray-200 dark:border-gray-700 pl-4">
            <div className="h-full flex flex-col">
              <GridFilterComponent
                data={waitersData}
                setFilteredData={setWaitersData}
      ...
      <Modal
        ...
        title={
          <div className="">
            <h3 className="text-lg font-semibold pl-4 ">Import Waiter</h3>
          </div>
        }
        size="xl"
        overlayProps={{
