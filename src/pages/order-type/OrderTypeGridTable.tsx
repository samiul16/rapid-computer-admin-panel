...
export const orderTypeMockData: OrderTypeType[] = [
  {
    id: "order-type-001",
    typeName: "Delivery",
  ...
  },
  {
    id: "order-type-003",
    typeName: "Dine-in",
    description: "Description for Dine-in",
...
export default function OrderTypeGridTable({
  setViewMode,
}: {
  ...
  const [gridData, setGridData] = useState(orderTypeMockData);
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [isExportOpen, setIsExportOpen] = useState(false);
  const [opened, { open, close }] = useDisclosure(false);
  const [modalData, setModalData] = useState({
    title: "Import Order Type",
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      typeName: `Order Type${gridData.length + index + 1}`,
      description: `Description${gridData.length + index + 1}`,
      customerId: `Customer${gridData.length + index + 1}`,
      requiresDeliveryman: Math.random() > 0.5,
      status:
        Math.random() > 0.3
          ? "active"
          : Math.random() > 0.5
    ...
    if (gridData.length >= 46) {
      ...
      setGridData((prev) => [...prev, ...(newItems as OrderTypeType[])]);
      setPage((prev) => prev + 1);
    }
  ...
  return (
    <div className="px-4 py-3 h-full flex flex-col bg-white dark:bg-gray-900 parent">
            ...
            {filteredGridData.map((item) => (
              ...
              >
                ...
                <div className="grid grid-cols-3 items-center gap-2 mb-4">
                  {/* Left - Title */}
                  <Tooltip label={item.typeName} position="top" withArrow>
                    <CardTitle
                      className="text-lg font-semibold cursor-pointer hover:text-primary transition-colors truncate"
                      onClick={() => navigate(`/order-type/1`)}
                    >
                      {item.typeName}
                    </CardTitle>
                  </Tooltip>
                  ...
                  <div className="flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        ...
                        onClick={() => {
                          ...
                          toastSuccess(
                            item.status === "active"
                              ...
                              : "Order Type inactivated successfully"
                          );
                        }}
                      >
                        {item.status === "active" ? (
                    ...
                    >
                      <div
                        ...
                        onClick={() => {
                          ...
                          toastSuccess(
                            item.isDeleted
                              ? "Order Type restored successfully"
                    ...
                    <Tooltip label="Edit" position="top" withArrow>
                      <div
                        ...
                        onClick={() => navigate(`/order-type/edit/1`)}
                      >
                ...
                <div className="grid grid-cols-2 items-center gap-4 pt-2 dark:border-gray-700">
                  ...
                  <div className="text-end">
                    <div className="text-[11px] text-gray-500 dark:text-gray-400">
                      Requires Deliveryman
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                      {item.requiresDeliveryman ? "Yes" : "No"}
                    </div>
                  </div>
                </div>
              </Card>
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">Loading more order types...</span>
              </div>
            </div>
          )}
      ...
      <Modal
        ...
        title={
          <div className="">
            <h3 className="text-lg font-semibold pl-4">Import Order Type</h3>
          </div>
        }
        size="xl"
        overlayProps={{
