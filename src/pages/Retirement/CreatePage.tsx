...
const initialData: RetirementData = {
  name: "",
  date: "",
  description: "",
  status: false,
...
export default function RetirementFormPage({ isEdit = false }: Props) {
  ...
  const name: boolean = usePermission("retirement", "create", "countryName");

  const isDefault: boolean = usePermission("retirement", "create", "isDefault");
  const isDraft: boolean = usePermission("retirement", "create", "isDraft");
  const flag: boolean = usePermission("retirement", "create", "flag");
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/retirement/create");
        } else {
          navigate("/retirement/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Retirement created successfully!");
      handleReset();
    } else {
      toastSuccess("Retirement created successfully!");
      navigate("/retirement");
  ...
  const handlePrintIncrements = (RetirementData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("RetirementData on pdf click", formData);
      const blob = await pdf(
        <GenericPDF
          data={[formData]}
          title="Retirement Details"
          subtitle="Retirement Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "retirement-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Retirement saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? labels.editingRetirement : labels.creatingRetirement}
        videoSrc={video}
              ...
              {canCreate && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    ...
                    labelText={labels.retirementNameTooltip}
                    tooltipText={labels.retirementNameTooltip}
                    required
                  />
                </div>
              )}
                ...
                <div className="md:col-span-3 space-y-2">
                  <div className="relative">
                    <EditableInput
                      ...
                      labelText={labels.retirementDateTooltip}
                      tooltipText={labels.retirementDateTooltip}
                      required
                      type="date"
                    />
                  </div>
                ...
                <div className="md:col-span-3 space-y-2">
                  <div className="relative">
                    <EditableInput
                      ...
                      labelText={labels.retirementDetailsTooltip}
                      tooltipText={labels.retirementDetailsTooltip}
                      required
                    />
                  </div>
                </div>
              ...
              {canCreate && (
                <div className="md:col-span-3 space-y-2 relative">
                  <Autocomplete
                    ...
                    value={formData.status === true ? labels.yes : labels.no}
                    ...
                    placeholder=" "
                    labelText={labels.retirementStatusTooltip}
                    className="relative"
                    styles={{
                      input: {
