...
const initialData: TerminationData = {
  date: "2024-02-15",
  iqamaNo: "1234567890",
...
// Termination Type options for autocomplete
...
export default function TerminationDetailsPage() {
  ...
  // get permission
  const canCreate: boolean = usePermission("termination", "create");
  const canView: boolean = usePermission("termination", "view");
  const canEdit: boolean = usePermission("termination", "edit");
  const canDelete: boolean = usePermission("termination", "delete");
  const canExport: boolean = usePermission("termination", "export");
  const canPdf: boolean = usePermission("termination", "pdf");
  const canPrint: boolean = usePermission("termination", "print");
  const canSeeHistory: boolean = usePermission("termination", "history");

  // Field-level permissions
  const canViewDate: boolean = usePermission("termination", "view", "date");
  const canViewIqamaNo: boolean = usePermission(
    "termination",
    "view",
    "iqamaNo"
  );
  const canViewBranch: boolean = usePermission("termination", "view", "branch");
  const canViewTerminationType: boolean = usePermission(
    "termination",
    "view",
    "terminationType"
  );
  ...
  const canViewStatus: boolean = usePermission("termination", "view", "status");
  ...
  // Get termination data based on selected termination type
  const getTerminationData = (terminationType: string): TerminationData => {
    const terminationMap: Record<string, TerminationData> = {
      "Voluntary Resignation": {
        date: "2024-02-15",
        iqamaNo: "1234567890",
        branch: "Riyadh Branch",
      ...
      "Contract Expiry": {
        ...
        description: "Contract ended naturally",
        status: "Pending",
        isActive: true,
        isDraft: false,
    ...
    return terminationMap[terminationType] || initialData;
  };

  const [terminationData, setTerminationData] = useState<TerminationData>(
    getTerminationData(selectedTerminationType)
  );
  ...
  // Update termination data when selection changes
  useEffect(() => {
    const newTerminationData = getTerminationData(selectedTerminationType);
    setTerminationData(newTerminationData);
    setSelectedBranch(newTerminationData.branch);
    setSelectedIqamaNo(newTerminationData.iqamaNo);
  ...
  const handlePrintTermination = (terminationData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("terminationData on pdf click", terminationData);
      const blob = await pdf(
        <GenericPDF
          data={[terminationData]}
          title="Termination Details"
          subtitle="Termination Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "termination-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Termination"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/termination")}
        listText="List"
        listPath="termination"
        activePage="view"
        popoverOptions={[
          {
            label: "Create",
          ...
          },
          {
            ...
            onClick: () => navigate("/termination/edit/1"),
          },
        ]}
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintTermination(terminationData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Date, Iqama No, Branch, Bonus Type */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            {canViewTerminationType && (
              <div className="mt-1">
                <Autocomplete
                  options={terminationTypeOptions}
                  value={selectedTerminationType}
                  onValueChange={setSelectedTerminationType}
                  placeholder="Select termination type..."
                  displayKey="terminationType"
                  valueKey="terminationType"
                  searchKey="terminationType"
                  disabled={false}
                  className="w-[96%] bg-gray-100 rounded-xl"
                  labelClassName="bg-gray-50 rounded-2xl"
            ...
            {canViewDate && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(terminationData.date)}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
            {canViewDescription && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(terminationData.description)}
                </div>
              </div>
            )}
            ...
            {canViewStatus && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(terminationData.status)}
                </div>
              </div>
            )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(terminationData.createdAt),
          updated: getRelativeTime(terminationData.updatedAt),
          drafted: getRelativeTime(terminationData.draftedAt),
          deleted: getRelativeTime(terminationData.deletedAt),
        }}
      />
