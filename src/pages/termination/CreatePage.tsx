...
export default function TerminationFormPage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("termination", "view");
  const canEdit = usePermission("termination", "edit");
  const canDelete = usePermission("termination", "delete");
  ...
  const date: boolean = usePermission("termination", "create", "date");
  const iqamaNo: boolean = usePermission("termination", "create", "iqamaNo");
  const branch: boolean = usePermission("termination", "create", "branch");
  const terminationType: boolean = usePermission(
    "termination",
    "create",
    "terminationType"
  );
  ...
  const status: boolean = usePermission("termination", "create", "status");
  const isDraft: boolean = usePermission("termination", "create", "isDraft");
  ...
  console.log("terminationType", terminationType);
  ...
  const statusOptions = ["Approved", "Pending", "Rejected", "Draft"];
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/termination/create");
        } else {
          navigate("/termination/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (printEnabled) {
      handlePrintTermination(formData);
    }

    // keep switch functionality
    if (keepCreating) {
      toastSuccess("Termination created successfully!");
      handleReset();
    } else {
      toastSuccess("Termination created successfully!");
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("terminationData on pdf click", formData);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Termination saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Termination" : "Creating Termination"}
        videoSrc={video}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              ...
              {terminationType && (
                <div className="space-y-2">
                  <div className="relative">
                    <Autocomplete
                      ref={(el: any) => setRef("terminationType")(el)}
                      id="terminationType"
                      ...
                      onValueChange={(value: string) => {
                        setFormData({ ...formData, terminationType: value });
                        if (value) {
              ...
              {description && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    tooltipText="Enter termination description"
                    required
                  />
                </div>
              )}
