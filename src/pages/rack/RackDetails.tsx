...
import { mockRackHistoryData } from "@/mockData/rack-history-mockdata";
...
export default function RackDetails() {
  ...
  const [initialSelectedValue, setInitialSelectedValue] = useState("RK-A1");
    ...
    try {
      ...
      const blob = await pdf(
        <PDF
          data={[viewData]}
          title="Rack Details"
          subtitle="Rack Information Report"
        />
      ).toBlob();
  ...
  return (
    <>
      <PageLayout
        title={t("button.viewingRack")}
        videoSrc={video}
        videoHeader="Rapid ERP Video"
        onListClick={() => navigate("/rack")}
        listText="List"
        listPath="/rack"
        popoverOptions={[
          {
            label: "Create",
            onClick: () => navigate("/rack/create"),
          },
          {
            label: "Edit",
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div className="md:col-span-4">
            <h3 className="font-medium mb-1">Rack Name</h3>
      ...
      >
        ...
        <Modal.Body>
          <HistoryDataTable columnData={mockRackHistoryData} />
        </Modal.Body>
      </Modal>
    </>
  );
