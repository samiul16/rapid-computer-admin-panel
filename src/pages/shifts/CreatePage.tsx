...
export default function ShiftCreatePage({ isEdit = false }: Props) {
  ...
  const canCreate: boolean = usePermission("shifts", "create");
  const canView: boolean = usePermission("shifts", "view");
  const canEdit: boolean = usePermission("shifts", "edit");
  const canDelete: boolean = usePermission("shifts", "delete");
  ...
  const isDefault: boolean = usePermission("shifts", "create", "isDefault");
  const isDraft: boolean = usePermission("shifts", "create", "isDraft");
  const canPdf: boolean = usePermission("shifts", "pdf");
  const canPrint: boolean = usePermission("shifts", "print");

  const permissionsFields = usePermission<keyof MockData>("shifts", "create", [
    "shift",
    "startTime",
    "endTime",
  ]);
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          ...
          navigate("/shifts/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Shifts created successfully!");
      handleReset();
    } else {
      toastSuccess("Shifts created successfully!");
      navigate("/shifts");
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Shifts saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? labels.editingShifts : labels.creatingShifts}
        videoSrc={video}
              ...
              {permissionsFields.shift && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    ...
                    onCancel={() => setFormData({ ...formData, shift: "" })}
                    labelText={labels.shift}
                    tooltipText={labels.shift}
                    required
                  />
                </div>
              )}
              ...
              {permissionsFields.startTime && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    ...
                    labelText={labels.startTimeTooltip}
                    tooltipText={labels.startTimeTooltip}
                    required
                    type="time"
                  />
                </div>
              ...
              {permissionsFields.endTime && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    ...
                    labelText={labels.endTimeTooltip}
                    tooltipText={labels.endTimeTooltip}
                    required
                    type="time"
                  />
                </div>
