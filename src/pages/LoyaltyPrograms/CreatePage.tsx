...
const initialData: MockDataType = {
  name: "",
  customerGroup: "",
  customer: "",
  startDate: "",
...
export default function LoyaltyProgramsCreatePage({ isEdit = false }: Props) {
  ...
  // Permission checks
  const canCreate = usePermission("loyaltyPrograms", "create");
  ...
  const date: boolean = usePermission("loyaltyPrograms", "create", "date");
  ...
  const permissionsFields = usePermission<
    keyof MockDataType | `PackageType.${keyof PackageType}`
  ...
  // Form state
  const [formData, setFormData] = useState<MockDataType>({
    name: "",
    customerGroup: "",
    customer: "",
    startDate: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("LoyaltyPrograms created successfully!");
      handleReset();
    } else {
      toastSuccess("LoyaltyPrograms created successfully!");
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[formData]}
          title="LoyaltyPrograms Details"
          subtitle="LoyaltyPrograms Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "LoyaltyPrograms-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("LoyaltyPrograms saved as draft successfully");
            },
