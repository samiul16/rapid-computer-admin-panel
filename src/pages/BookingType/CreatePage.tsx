...
const initialData: RoomSizeData = {
  bookingType: "",
  description: "",
  isDefault: false,
  isActive: true,
...
export default function BookingTypeCreatePage({ isEdit = false }: Props) {
  ...
  const name: boolean = usePermission("bookingType", "create", "name");

  const isDefault: boolean = usePermission("bookingType", "create", "isDefault");
  const isDraft: boolean = usePermission("bookingType", "create", "isDraft");
  ...
  // Form state
  const [formData, setFormData] = useState<RoomSizeData>({
    bookingType: "",
    description: "",
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/booking-type/create");
        } else {
          navigate("/booking-type/edit/undefined");
        }
      },
  ...
  const handleSwitchChange = (checked: boolean) => {
    ...
    //   setTimeout(() => handlePrintRoomSize(formData), 100);
    // }
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("RoomSizeData on pdf click", formData);
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? labels.editingBookingType : labels.creatingBookingType}
        videoSrc={video}
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 my-8 relative">
              {canCreate && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    setRef={setRef("bookingType")}
                    id="bookingType"
                    name="bookingType"
                    value={formData.bookingType}
                    onChange={handleChange}
                    onNext={() => focusNextInput("description")}
                    onCancel={() => setFormData({ ...formData, bookingType: "" })}
                    labelText={labels.bookingTypeTooltip}
                    tooltipText={labels.bookingTypeTooltip}
                    required
                  />
                </div>
              )}
