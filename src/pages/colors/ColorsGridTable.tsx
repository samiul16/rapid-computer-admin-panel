...
export default function ColorsGridTable({
  setViewMode,
}: {
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      name: `Color Name#${gridData.length + index + 1}`,
  ...
  return (
    <div className="px-4 py-3 h-full flex flex-col bg-white dark:bg-gray-900 parent">
                        ...
                        onClick={() => {
                          ...
                          toastSuccess(
                            item.status === "active"
                              ...
                              : "Color inactivated successfully"
                          );
                        }}
                      >
                        {item.status === "active" ? (
      ...
      <Modal
        ...
        title={
          <div className="">
            <h3 className="text-lg font-semibold pl-4">Import Colors</h3>
          </div>
        }
        size="xl"
        overlayProps={{
