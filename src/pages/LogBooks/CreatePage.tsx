...
export default function LogBooksCreatePage({ isEdit = false }: Props) {
  ...
  // const name: boolean = usePermission("logBooks", "create", "name");
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/log-books/create");
        } else {
          navigate("/log-books/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Log Books created successfully!");
      handleReset();
    } else {
      toastSuccess("Log Books created successfully!");
      navigate("/log-books");
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[formData]}
          title="Log Details"
          subtitle="Log Books Information"
        />
      ).toBlob();
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Log Books saved as draft successfully");
            },
