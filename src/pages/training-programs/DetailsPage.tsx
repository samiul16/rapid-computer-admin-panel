...
export default function TrainingProgramsDetails() {
  ...
  // get permission
  // const canCreate: boolean = usePermission("trainingPrograms", "create");
  // const canView: boolean = usePermission("trainingPrograms", "view");
  // const canEdit: boolean = usePermission("trainingPrograms", "edit");
  // const canDelete: boolean = usePermission("trainingPrograms", "delete");
  // const canExport: boolean = usePermission("trainingPrograms", "export");
  const canPdf: boolean = usePermission("trainingPrograms", "pdf");
  const canPrint: boolean = usePermission("trainingPrograms", "print");
  const canSeeHistory: boolean = usePermission("trainingPrograms", "history");
  ...
  const getLeavesData = (leaveType: string): TermsData => {
    const leavesMap: Record<string, TermsData> = {
      "React.js Fundamentals": initialData,
      "Node.js Fundamentals": initialData,
      "Python Fundamentals": initialData,
    };
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[leavesData]}
          title="Training Programs Details"
          subtitle="Training Programs Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "training-programs-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Training Programs"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/training-programs")}
        listText="List"
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/training-programs/create"),
          },
          {
            label: "Edit",
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.trainingProgramName && (
              <div className="mt-1">
                <Autocomplete
                  ...
                  placeholder=" "
                  displayKey="trainingProgramName"
                  valueKey="trainingProgramName"
                  searchKey="trainingProgramName"
                  disabled={false}
            ...
            {fieldPermissions.trainingType && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Training Type
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.trainingType)}
                </div>
              </div>
            )}
            {fieldPermissions.programItems && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Program Items
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.programItems)}
                </div>
              </div>
            )}
            {fieldPermissions.point && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Point</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.point)}
                </div>
              </div>
            )}
            {fieldPermissions.trainingMode && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Training Mode
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.trainingMode)}
                </div>
              </div>
            )}
            {fieldPermissions.departments && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Departments</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.departments)}
                </div>
              </div>
            )}
            {fieldPermissions.applyPosition && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Apply Position
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.applyPosition)}
                </div>
              </div>
            )}
