...
export default function ContractDetails() {
  ...
  // Get contract data based on selected contract
  const getContractData = (contractName: string): DetailsPageTypes => {
    const contractMap: Record<string, DetailsPageTypes> = {
      "Software Development Agreement": initialData,
      "Cloud Infrastructure Setup": initialData,
      "Mobile App Development": initialData,
      "Security Audit Services": initialData,
      "Database Migration Project": initialData,
      "API Integration Services": initialData,
      "UI/UX Design Services": initialData,
      "Performance Optimization": initialData,
      "Testing Framework Implementation": initialData,
      "Documentation Services": initialData,
      "Maintenance Contract": initialData,
      "Master Service Agreement": initialData,
    };

    return contractMap[contractName] || initialData;
  };

  const [contractData, setContractData] = useState<DetailsPageTypes>(
    getContractData(selectedContract)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update contract data when selection changes
  useEffect(() => {
    setContractData(getContractData(selectedContract));
  }, [selectedContract]);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", contractData);
  ...
  return (
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintContract(contractData);
                }
              }
            : undefined
        }
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((contractData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(contractData.createdAt),
          updated: getRelativeTime(contractData.updatedAt),
          drafted: getRelativeTime(contractData.draftedAt),
          deleted: getRelativeTime(contractData.deletedAt),
        }}
      />
