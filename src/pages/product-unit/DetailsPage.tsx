...
export default function DoctorDetails() {
  ...
  // Get product unit data based on selected product unit
  const getProductUnitData = (productUnitName: string): DetailsPageTypes => {
    const productUnitMap: Record<string, DetailsPageTypes> = {
      Kilogram: {
        ...initialData,
        unitTitle: "Kilogram",
    ...
    return productUnitMap[productUnitName] || initialData;
  };

  const [productUnitData, setProductUnitData] = useState<DetailsPageTypes>(
    getProductUnitData(selectedProductUnit)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update product unit data when selection changes
  useEffect(() => {
    setProductUnitData(getProductUnitData(selectedProductUnit));
  }, [selectedProductUnit]);
  ...
  const handlePrintProductUnit = (productUnitData: any) => {
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          ...
          isDefault: "Default Product Unit",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", productUnitData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintProductUnit(productUnitData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Product Unit Selection, Product Unit Details */}
            ...
            {fieldPermissions.slNo && (
              <div className="mt-1">
                <Autocomplete
                  ...
                  onValueChange={setSelectedProductUnit}
                  placeholder=" "
                  displayKey="name"
                  valueKey="name"
                  searchKey="name"
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((productUnitData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(productUnitData.createdAt),
          updated: getRelativeTime(productUnitData.updatedAt),
          drafted: getRelativeTime(productUnitData.draftedAt),
          deleted: getRelativeTime(productUnitData.deletedAt),
        }}
      />
