...
export default function GoodsCreatePage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("goods", "view");
  const canEdit = usePermission("goods", "edit");
  const canDelete = usePermission("goods", "delete");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Goods created successfully!");
      handleReset();
    } else {
      toastSuccess("Goods created successfully!");
      navigate("/goods");
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Goods saved as draft successfully");
            },
  ...
  const handleImageFile = (file: File) => {
    if (file.type.match("image.*")) {
      ...
      reader.onload = (e) => {
        ...
        setFormData({ ...formData, piNoImage: e.target?.result as string });
      };
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Goods" : "Creating Goods"}
        videoSrc={video}
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              ...
              {permissionsFieldLevel.netWeight && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("netWeight")}
                    type="text"
                    id="netWeight"
                    name="netWeight"
                    value={formData.netWeight}
                    onChange={handleChange}
                    onNext={() => focusNextInput("totalCTN")}
                    onCancel={() => setFormData({ ...formData, netWeight: "" })}
                    labelText="Net Weight"
                    tooltipText="Enter net weight"
                    required
                  />
                </div>
              )}
              ...
              {permissionsFieldLevel.totalCTN && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("totalCTN")}
                    type="text"
                    id="totalCTN"
                    name="totalCTN"
                    value={formData.totalCTN}
                    onChange={handleChange}
                    onNext={() => focusNextInput("totalPCS")}
                    onCancel={() => setFormData({ ...formData, totalCTN: "" })}
                    labelText="Total CTN"
                    tooltipText="Enter total CTN"
                    required
                  />
                </div>
              )}
              ...
              {permissionsFieldLevel.totalPCS && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("totalPCS")}
                    type="text"
                    id="totalPCS"
                    name="totalPCS"
                    value={formData.totalPCS}
                    onChange={handleChange}
                    onNext={() => focusNextInput("artWork")}
                    onCancel={() => setFormData({ ...formData, totalPCS: "" })}
                    labelText="Total PCS"
                    tooltipText="Enter total PCS"
                    required
                  />
                </div>
              )}
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      ...
                      if (imagePreview) {
                        ...
                        setFormData({ ...formData, piNoImage: null });
                        setTimeout(() => {
                          triggerFileInput();
                        }, 0);
                      } else {
