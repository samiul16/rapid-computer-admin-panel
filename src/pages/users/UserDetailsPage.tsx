...
const USER_DATA = MOCK_USERS.map((user) => user.id);
...
export default function UserDetailsPage() {
  ...
  const userData: UserData = {
    name: MOCK_USERS.find((u) => u.id === selectedUser)?.name || "John Smith",
    isDefault: true,
    isActive: true,
    isDraft: false,
    isDeleted: false,
  ...
  const handleSwitchChange = (checked: boolean) => {
    ...
    if (checked) {
      setTimeout(() => handlePrintUser(userData), 100);
    }
  };
      ...
      <div className="flex flex-col h-[82vh] overflow-hidden border rounded shadow bg-white dark:bg-gray-800 ">
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-primary">Viewing User</h1>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center">
              <SplitButton
                onListClick={() => navigate("/users")}
                listText="List"
                listPath="/users"
                popoverOptions={[
                  {
