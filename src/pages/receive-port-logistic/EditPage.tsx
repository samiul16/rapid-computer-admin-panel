...
const initialData: ReceivePortLogisticData = {
  country: "Saudi Arabia",
  company: "Al-Rashid Trading Company",
  note: "Sample note for editing",
  isActive: true,
  createdAt: new Date(),
  updatedAt: new Date(),
  isDeleted: false,
...
export default function TransitOrderEditPage({ isEdit = true }: Props) {
  ...
  const canView = usePermission("receivePortLogistic", "view");
  const canEdit = usePermission("receivePortLogistic", "edit");
  const canDelete = usePermission("receivePortLogistic", "delete");
  ...
  const permissionsFields = usePermission<keyof ReceivePortLogisticData>(
    "receivePortLogistic",
    "edit",
    ["country", "company", "note"]
  );

  const canPdf: boolean = usePermission("receivePortLogistic", "pdf");
  const canPrint: boolean = usePermission("receivePortLogistic", "print");
  ...
  // Form state
  const [formData, setFormData] = useState<ReceivePortLogisticData>({
    country: "",
    company: "",
    note: "",
    isActive: true,
  ...
  const [popoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/receive-port-logistic/create");
        } else {
          navigate("/receive-port-logistic/edit/undefined");
        }
      },
    ...
    },
    {
      ...
      onClick: () => {
        navigate("/receive-port-logistic/view");
      },
      // Only show if user has permission
      show: canView,
    },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Transit order record edited successfully!");
      handleReset();
    } else {
      toastSuccess("Transit order record edited successfully!");
      navigate("/receive-port-logistic");
    }
  };
  ...
  return (
    <>
      <PageLayout
        title={
          isEdit
            ? "Editing Receive Port Logistic"
            : "Creating Receive Port Logistic"
        }
        videoSrc={video}
        videoHeader="Tutorial video"
        listPath="receive-port-logistic"
