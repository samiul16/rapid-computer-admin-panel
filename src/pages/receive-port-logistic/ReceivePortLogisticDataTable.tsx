...
const mockReceivePortLogistics = [
  {
    id: "1",
    sn: "001",
...
export default function ReceivePortLogisticDataTable({
  viewMode,
  setViewMode,
  dataTableFilter,
  searchQuery,
  ...
  const canCreate = usePermission("receivePortLogistic", "create");
  const canViewSn = usePermission("receivePortLogistic", "view", "sn");
  ...
  const canViewPiNo = usePermission("receivePortLogistic", "view", "piNo");
  ...
  const canViewStatus = usePermission("receivePortLogistic", "view", "status");
  const canViewDate = usePermission("receivePortLogistic", "view", "date");
  ...
  const componentColumns = [
    {
      ...
      title: "SN",
      options: canViewSn
        ? [...new Set(mockReceivePortLogistics.map((item) => item.sn))]
        : [],
    ...
    },
    {
      ...
      options: canViewCountry
        ? [...new Set(mockReceivePortLogistics.map((item) => item.country))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
    ...
    },
    {
      ...
      options: canViewCompany
        ? [...new Set(mockReceivePortLogistics.map((item) => item.company))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
    ...
    },
    {
      ...
      title: "P.I.No",
      options: canViewPiNo
        ? [...new Set(mockReceivePortLogistics.map((item) => item.piNo))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
    ...
    },
    {
      ...
      options: canViewInvoiceNo
        ? [...new Set(mockReceivePortLogistics.map((item) => item.invoiceNo))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
    ...
    },
    {
      ...
      options: canViewSupplierName
        ? [
            ...new Set(
              mockReceivePortLogistics.map((item) => item.supplierName)
            ),
          ]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
    ...
    },
    {
      ...
      options: canViewStatus
        ? [...new Set(mockReceivePortLogistics.map((item) => item.status))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
    ...
    },
    {
      ...
      title: "Date",
      options: canViewDate
        ? [...new Set(mockReceivePortLogistics.map((item) => item.date))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
    ...
    },
    {
      ...
      options: canViewLoginId
        ? [...new Set(mockReceivePortLogistics.map((item) => item.loginId))]
        : [],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
  ...
  const filteredData = mockReceivePortLogistics.filter(
    (receivePortLogistic) => {
      if (dataTableFilter.status === "Active") {
        return receivePortLogistic.isActive;
      } else if (dataTableFilter.status === "Inactive") {
        return !receivePortLogistic.isActive;
      } else if (dataTableFilter.status === "Deleted") {
        return receivePortLogistic.isDeleted;
      }
      return true;
    }
  );
  ...
  return (
    <FixedColumnDataTable
      ...
      pathName="receive-port-logistic"
