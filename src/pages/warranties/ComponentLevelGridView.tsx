...
const warrantyData: GridDataType[] = [
  {
    id: "1",
    claimCode: "CLM-1001",
...
export default function ComponentLevelGridView({
  ...
  const [gridData, setGridData] = useState(warrantyData);
  const canDelete: boolean = usePermission("warranties", "delete");
  const canRestore: boolean = usePermission("warranties", "restore");
  const canEdit: boolean = usePermission("warranties", "edit");

  // Debug permissions
  console.log("warranties Permissions:", {
    canDelete,
    canRestore,
    canEdit,
  });
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      ...
      claimCode: `Claim Code ${index + 1}`,
      dateCreated: `Date Created ${index + 1}`,
      customer: `Customer ${index + 1}`,
      invoice: `Invoice ${index + 1}`,
      productService: `Product Service ${index + 1}`,
      receiptProcess: `Receipt Process ${index + 1}`,
  ...
  const filteredData = gridData.filter(
    (item) =>
      item.claimCode.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      item.productService.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.receiptProcess.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Total {gridData.length} Warranties
        </span>
      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (canRestore && item.isDeleted) {
                            ...
                            toastRestore("Warranty restored successfully");
                          } else {
                            if (canDelete) {
                              handleDeleteClick(item.id);
                              toastDelete("Warranty deleted successfully");
                            }
                          }
                        }}
                      >
