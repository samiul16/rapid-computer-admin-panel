...
export default function ShortcutDetails() {
  ...
  // Get facilitydetails data based on selected facility
  const getFacilityDetailsData = (facilityName: string): DetailsPageTypes => {
    const facilitydetailsMap: Record<string, DetailsPageTypes> = {
      "Main Office Complex": initialData,
      "Production Facility A": initialData,
      "R&D Laboratory": initialData,
      "Distribution Center": initialData,
      "Customer Support Hub": initialData,
      "Learning Academy": initialData,
      "IT Infrastructure Hub": initialData,
      "Testing Laboratory": initialData,
      "Employee Wellness Hub": initialData,
      "Maintenance Facility": initialData,
      "Regional Office": initialData,
      "Assembly Line B": initialData,
    };

    return facilitydetailsMap[facilityName] || initialData;
  };

  const [facilitydetailsData, setFacilitydetailsData] =
    useState<DetailsPageTypes>(getFacilityDetailsData(selectedFacility));

  const inputRef = useRef<HTMLInputElement>(null);

  // Update facilitydetails data when selection changes
  useEffect(() => {
    setFacilitydetailsData(getFacilityDetailsData(selectedFacility));
  }, [selectedFacility]);
  ...
  const handlePrintFacilityDetails = (facilitydetailsData: any) => {
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          ...
          isDefault: "Default Facility Details",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", facilitydetailsData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintFacilityDetails(facilitydetailsData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Facility Selection, Facility Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.indexName && (
              <div className="mt-1">
                <Autocomplete
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((facilitydetailsData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(facilitydetailsData.createdAt),
          updated: getRelativeTime(facilitydetailsData.updatedAt),
          drafted: getRelativeTime(facilitydetailsData.draftedAt),
          deleted: getRelativeTime(facilitydetailsData.deletedAt),
        }}
      />
