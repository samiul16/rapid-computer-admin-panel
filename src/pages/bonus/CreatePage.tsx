...
export default function BonusFormPage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("bonus", "view");
  const canEdit = usePermission("bonus", "edit");
  const canDelete = usePermission("bonus", "delete");
  ...
  const date: boolean = usePermission("bonus", "create", "date");
  const iqamaNo: boolean = usePermission("bonus", "create", "iqamaNo");
  const branch: boolean = usePermission("bonus", "create", "branch");
  const bonusType: boolean = usePermission("bonus", "create", "bonusType");
  const bonusAmount: boolean = usePermission("bonus", "create", "bonusAmount");
  const notes: boolean = usePermission("bonus", "create", "notes");
  const isDraft: boolean = usePermission("bonus", "create", "isDraft");
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("bonusData on pdf click", formData);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Bonus saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Bonus" : "Creating Bonus"}
        videoSrc={video}
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              ...
              {bonusType && (
                <div className="space-y-2">
                  <div className="relative">
                    <Autocomplete
                      ref={(el: any) => setRef("bonusType")(el)}
                      id="bonusType"
                      name="bonusType"
                      allowCustomInput={true}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              ...
              {bonusAmount && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("bonusAmount")}
                    id="bonusAmount"
                    name="bonusAmount"
                    type="number"
                    value={formData.bonusAmount.toString()}
                    onChange={(e) => {
                      const value = parseFloat(e.target.value) || 0;
                      setFormData({ ...formData, bonusAmount: value });
                    }}
                    onNext={() => focusNextInput("notes")}
                    onCancel={() =>
                      setFormData({ ...formData, bonusAmount: 0 })
                    }
                    labelText="Bonus Amount"
                    tooltipText="Enter bonus amount"
                    required
                  />
                </div>
              )}
