...
export default function BonusDetailsPage() {
  ...
  // get permission
  const canCreate: boolean = usePermission("bonus", "create");
  const canView: boolean = usePermission("bonus", "view");
  const canEdit: boolean = usePermission("bonus", "edit");
  const canDelete: boolean = usePermission("bonus", "delete");
  const canExport: boolean = usePermission("bonus", "export");
  const canPdf: boolean = usePermission("bonus", "pdf");
  const canPrint: boolean = usePermission("bonus", "print");
  const canSeeHistory: boolean = usePermission("bonus", "history");

  // Field-level permissions
  const canViewDate: boolean = usePermission("bonus", "view", "date");
  const canViewIqamaNo: boolean = usePermission("bonus", "view", "iqamaNo");
  const canViewBranch: boolean = usePermission("bonus", "view", "branch");
  const canViewBonusType: boolean = usePermission("bonus", "view", "bonusType");
  const canViewBonusAmount: boolean = usePermission(
    "bonus",
    "view",
    "bonusAmount"
  );
  const canViewNotes: boolean = usePermission("bonus", "view", "notes");
  const canViewIsActive: boolean = usePermission("bonus", "view", "isActive");
  const canViewIsDraft: boolean = usePermission("bonus", "view", "isDraft");
  const canViewIsDeleted: boolean = usePermission("bonus", "view", "isDeleted");
  ...
  // Get bonus data based on selected bonus type
  const getBonusData = (bonusType: string): BonusData => {
    const bonusMap: Record<string, BonusData> = {
      "Performance Bonus": {
        date: "2024-02-15",
        iqamaNo: "1234567890",
        branch: "Riyadh Branch",
    ...
    return bonusMap[bonusType] || initialData;
  };

  const [bonusData, setBonusData] = useState<BonusData>(
    getBonusData(selectedBonusType)
  );
  ...
  useEffect(() => {
    const newBonusData = getBonusData(selectedBonusType);
    setBonusData(newBonusData);
    setSelectedBranch(newBonusData.branch);
    setSelectedIqamaNo(newBonusData.iqamaNo);
  }, [selectedBonusType]);
  ...
  const handlePrintBonus = (bonusData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("bonusData on pdf click", bonusData);
      const blob = await pdf(
        <GenericPDF
          data={[bonusData]}
          title="Bonus Details"
          subtitle="Bonus Information"
        />
      ).toBlob();
  ...
  return (
    <>
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/bonus/create"),
          },
          {
            label: "Edit",
            icon: <Edit className="w-5 h-5 text-blue-600" />,
            onClick: () => navigate("/bonus/edit/1"),
          },
        ]}
            ...
            {canViewDate && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(bonusData.date)}
                </div>
              </div>
            )}
          </div>

          {/* Row 2: Bonus Amount, Notes */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
            {canViewBonusAmount && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Bonus Amount</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(bonusData.bonusAmount)}
                </div>
              </div>
            )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(bonusData.createdAt),
          updated: getRelativeTime(bonusData.updatedAt),
          drafted: getRelativeTime(bonusData.draftedAt),
          deleted: getRelativeTime(bonusData.deletedAt),
        }}
      />
