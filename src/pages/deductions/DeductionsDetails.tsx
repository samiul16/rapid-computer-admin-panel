...
export default function DeductionsDetails() {
  const navigate = useNavigate();
  const { isRTL } = useSelector((state: RootState) => state.language);

  const [keepChanges, setKeepChanges] = useState(false);
  const [isOptionModalOpen, setIsOptionModalOpen] = useState(false);
  const [selectedDeductionIqamaNo, setSelectedDeductionIqamaNo] =
    useState("1234567890");
  ...
  // get permission
  const canCreate: boolean = usePermission("deductions", "create");
  const canView: boolean = usePermission("deductions", "view");
  const canEdit: boolean = usePermission("deductions", "edit");
  const canDelete: boolean = usePermission("deductions", "delete");
  const canExport: boolean = usePermission("deductions", "export");
  const canPdf: boolean = usePermission("deductions", "pdf");
  const canPrint: boolean = usePermission("deductions", "print");
  const canSeeHistory: boolean = usePermission("deductions", "history");
  ...
  //   "iqamaNo"
  // );
  const canViewDeductionType: boolean = usePermission(
    "deductions",
    "view",
    "deductionType"
  );
  const canViewBranch: boolean = usePermission("deductions", "view", "branch");
  const canViewDeductionAmount: boolean = usePermission(
    "deductions",
    "view",
    "deductionAmount"
  );
  const canViewNotes: boolean = usePermission("deductions", "view", "notes");
  const canViewDate: boolean = usePermission("deductions", "view", "date");
  ...
  const [leavesData, setLeavesData] = useState<DeductionsData>(
    getLeavesData(selectedDeductionIqamaNo)
  );
  ...
  useEffect(() => {
    setLeavesData(getLeavesData(selectedDeductionIqamaNo));
  }, [selectedDeductionIqamaNo]);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Deduction"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/deductions")}
        listText="List"
        listPath="deductions"
        activePage="view"
        popoverOptions={[
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {canViewDeductionType && (
              <div className="mt-1">
                <Autocomplete
                  options={deductionIqamaNoOptions}
                  value={selectedDeductionIqamaNo}
                  onValueChange={setSelectedDeductionIqamaNo}
                  placeholder=" "
                  displayKey="iqamaNo"
                  valueKey="iqamaNo"
                  searchKey="iqamaNo"
            ...
            {canViewDeductionAmount && (
              <div className="">
                ...
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.deductionAmount)}
                </div>
              </div>
            )}
