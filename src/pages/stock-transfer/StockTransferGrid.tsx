...
export default function StockTransferGrid({
  ...
  const loadMoreData = useCallback(async () => {
    ...
    if (stockData.length >= 46) {
      ...
      setStockData((prev: any) => [...prev, ...newItems]);
      setPage((prev) => prev + 1);
    }
  ...
  const filteredStock = stockData.filter(
    (stock) =>
      ...
      stock.sourceBranch.toLowerCase().includes(searchQuery.toLowerCase()) ||
      stock.destinationBranch
        .toLowerCase()
        ...
        >
          <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 pb-4">
            {filteredStock.map((stock) => (
              ...
              >
                {/* Top Row - Branches with Transfer Arrow */}
