...
export default function FacilityDetails() {
  ...
  // Get facility data based on selected facility
  const getFacilityData = (facilityName: string): DetailsPageTypes => {
    const facilityMap: Record<string, DetailsPageTypes> = {
      "Main Office Building": initialData,
      "Warehouse Facility A": initialData,
      "Manufacturing Plant B": initialData,
      "Research Laboratory": initialData,
      "Distribution Center": initialData,
      "Customer Service Center": initialData,
      "Data Center Facility": initialData,
      "Training Center": initialData,
      "Maintenance Workshop": initialData,
      "Storage Facility": initialData,
      "Production Floor": initialData,
      "Administrative Building": initialData,
    };

    return facilityMap[facilityName] || initialData;
  };

  const [facilityData, setFacilityData] = useState<DetailsPageTypes>(
    getFacilityData(selectedFacility)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update facility data when selection changes
  useEffect(() => {
    setFacilityData(getFacilityData(selectedFacility));
  }, [selectedFacility]);
  ...
  const handlePrintFacility = (facilityData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", facilityData);
  ...
  return (
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintFacility(facilityData);
                }
              }
            : undefined
        }
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((facilityData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(facilityData.createdAt),
          updated: getRelativeTime(facilityData.updatedAt),
          drafted: getRelativeTime(facilityData.draftedAt),
          deleted: getRelativeTime(facilityData.deletedAt),
        }}
      />
