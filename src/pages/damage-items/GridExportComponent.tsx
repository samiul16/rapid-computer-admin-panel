...
export default function OpeningStockExportComponent({
  ...
  const formatDataForExport = (rawData: any[]) => {
    return rawData.map((item) => ({
      "Item ID": item.itemId,
      "Quantity Damaged": item.quantityDamaged,
      "Damage Date": new Date(item.damageDate).toLocaleDateString(),
  ...
  const handleCSV = () => {
    ...
    exportToCSV(formattedData, "damage-items.csv");
  };

  const handleExcel = () => {
    const formattedData = formatDataForExport(mockOpeningStockData);
    exportToExcel(formattedData, "damage-items.xlsx");
  };
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      ...
      const printContent = `
        <html>
          <head>
            <title>Damage Items Report</title>
            <style>
