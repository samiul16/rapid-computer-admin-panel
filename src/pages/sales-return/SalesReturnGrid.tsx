...
// Define Sales Return interface based on the specified data structure
...
export default function SalesReturnGrid({
  searchQuery,
  setIsFilterOpen,
  isFilterOpen,
  setIsExportOpen,
  ...
  console.log("Sales Return grid rendered");
  ...
  const [salesReturnsData, setSalesReturnsData] = useState(salesReturns);
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => {
      const seq = salesReturnsData.length + index + 1;
      return {
        id: `${Date.now()}-${index}`,
        documentNumber: `SR${seq.toString().padStart(3, "0")}`,
        salesInvoiceNumber: `INV-2024-${seq.toString().padStart(3, "0")}`,
        poNumber: `PO-2024-${seq.toString().padStart(3, "0")}`,
        poDate: new Date(),
        ...
        remarks: `Sales return remarks ${seq}`,
    ...
    if (salesReturnsData.length >= 46) {
      setHasMore(false);
    } else {
      setSalesReturnsData((prev) => [...prev, ...newItems]);
      setPage((prev) => prev + 1);
    }

    setIsLoading(false);
  }, [salesReturnsData.length, isLoading, hasMore]);
  ...
  // Filter sales returns based on search query
  const filteredSalesReturns = salesReturnsData.filter((salesReturn) => {
    const q = searchQuery.toLowerCase();
    return (
      salesReturn.customer.toLowerCase().includes(q) ||
      salesReturn.documentNumber.toLowerCase().includes(q) ||
      salesReturn.salesInvoiceNumber.toLowerCase().includes(q) ||
      salesReturn.poNumber.toLowerCase().includes(q) ||
      salesReturn.paymentMode.toLowerCase().includes(q) ||
      salesReturn.vatNumber.toLowerCase().includes(q) ||
      salesReturn.salesman.toLowerCase().includes(q) ||
      salesReturn.city.toLowerCase().includes(q) ||
      salesReturn.state.toLowerCase().includes(q) ||
      salesReturn.country.toLowerCase().includes(q) ||
      salesReturn.remarks.toLowerCase().includes(q)
    );
  });

  const handleViewClick = (salesReturnId: string) => {
    const viewMode = searchParams.get("view") || "grid";
    navigate(`/sales-return/view/${salesReturnId}?fromView=${viewMode}`);
  };
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          >
            {filteredSalesReturns.map((salesReturn, index) => (
              <Card
                ...
                onClick={() => handleViewClick(salesReturn.id)}
              >
                ...
                <div className="grid grid-cols-2 gap-2 w-full">
                  ...
                  <div className="text-right min-w-0">
                    ...
                    <div className="text-sm font-normal text-gray-900 dark:text-gray-100 truncate">
                      {salesReturn.salesInvoiceNumber}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 items-center justify-between gap-2 w-full dark:border-gray-700">
                  ...
                  <div className="text-right min-w-0">
                    ...
                    <div className="text-sm font-normal text-gray-900 dark:text-gray-100 truncate">
                      {salesReturn.poDate.toLocaleDateString()}
                    </div>
                  </div>
                </div>
          ...
          {!hasMore && filteredSalesReturns.length > 12 && (
            <div className="flex justify-center items-center py-8">
          ...
          <div className={cn("h-full", isMobile ? "pb-4 mt-1" : "p-2")}>
            ...
            >
              <GridFilterComponent
                ...
                data={salesReturns}
                setFilteredData={setSalesReturnsData}
