...
interface Invoice {
  ...
  salesInvoiceNumber: string; // was invoiceNumber
  poNumber: string;
  poDate: Date;
  customer: string;
  vatNumber: string; // was trnNumber
  paymentMode: string;
  dueDays: number;
  paymentDate: Date;
  country: string;
...
export default function SalesReturnGrid({
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => {
      ...
      return {
        ...
        documentNumber: `DOC${(invoicesData.length + index + 1)
          .toString()
          .padStart(3, "0")}`,
        salesInvoiceNumber: `SI-2024-${(invoicesData.length + index + 1)
          .toString()
          .padStart(3, "0")}`,
        poNumber: `PO-${(invoicesData.length + index + 1)
          .toString()
          .padStart(4, "0")}`,
        poDate: new Date(Date.now() + Math.random() * 90 * 24 * 60 * 60 * 1000),
        customer: `Customer ${invoicesData.length + index + 1}`,
        vatNumber: `VAT${(invoicesData.length + index + 1)
          .toString()
          .padStart(9, "0")}`,
  ...
  const filteredInvoices = invoicesData.filter(
    (invoice) =>
      ...
      invoice.vatNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
