...
const DynamicInputTableList = ({ isEdit = false, onFormRowsChange }: Props) => {
  ...
  const LAST_FIELD = "description";
  const FIRST_FIELD = "taskCategory";
  ...
  const handleAddRowAndFocus = () => {
    ...
    if (!lastRow.taskCategory || lastRow.taskCategory.trim() === "") {
      if (listRef.current) {
  ...
  const handleItemChange = (rowIdx: number, value: string | null) => {
    setFormRows((prev) =>
      prev.map((row, idx) =>
        idx === rowIdx ? { ...row, taskCategory: value ?? "" } : row
      )
    );
    focusNextInput(rowIdx, "customer");
  };

  const handleCustomerChange = (rowIdx: number, value: string | null) => {
    setFormRows((prev) =>
      prev.map((row, idx) =>
        idx === rowIdx ? { ...row, customer: value ?? "" } : row
      )
    );
    focusNextInput(rowIdx, "project");
  };

  const handleProjectChange = (rowIdx: number, value: string | null) => {
    setFormRows((prev) =>
      prev.map((row, idx) =>
        idx === rowIdx ? { ...row, project: value ?? "" } : row
      )
    );
