...
export default function BusinessBrokerDataTable({
  viewMode,
  setViewMode,
  dataTableFilter,
  searchQuery,
  ...
  const componentColumns = [
    ...
    },
    {
      ...
      title: "Hourly Rate",
      options: [...new Set(mockTableData.map((item) => item.hourlyRate))],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
        return filterValue.some((filterVal: string) =>
      ...
      sortingFn: (row1: any, row2: any) => {
        return row1
          .getValue("hourlyRate")
          .localeCompare(row2.getValue("hourlyRate"));
      },
      size: 150,
      minSize: 180,
      meta: {
