...
export const deliverymenData: DeliverymanType[] = [
  {
    id: "dm-001",
    code: "DM001",
    name: "Rahim Uddin",
...
export default function DeliverymenGridTable({
  setViewMode,
}: {
  setViewMode: (viewMode: "grid" | "list") => void;
}) {
  console.log("Deliverymen grid rendered");
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState("");
  const [gridData, setGridData] = useState(deliverymenData);
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [isExportOpen, setIsExportOpen] = useState(false);
  const [opened, { open, close }] = useDisclosure(false);
  const [modalData, setModalData] = useState({
    title: "Import Deliverymen",
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      name: `Deliverymen${gridData.length + index + 1}`,
      code: `DM${gridData.length + index + 1}`,
      phoneNumber: `01711112222`,
      email: `dm${gridData.length + index + 1}@example.com`,
      address: `Address${gridData.length + index + 1}`,
      nationalId: `1234567890`,
      image: `/customer-dummy-image.jpg`,
      vehicleType: "Bike",
      assignedArea: `Area${gridData.length + index + 1}`,
      status:
        Math.random() > 0.3
          ? "active"
          : Math.random() > 0.5
    ...
    if (gridData.length >= 46) {
      ...
      setGridData((prev) => [...prev, ...(newItems as DeliverymanType[])]);
      setPage((prev) => prev + 1);
    }
  ...
  const filteredGridData = gridData.filter(
    (item) =>
      ...
      item.assignedArea?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="px-4 py-3 h-full flex flex-col bg-white dark:bg-gray-900 parent">
                ...
                <div className="grid grid-cols-3 items-center gap-2 mb-4">
                  ...
                  <Tooltip label={item.name} position="top" withArrow>
                    <CardTitle
                      ...
                      onClick={() => navigate(`/deliverymen/1`)}
                    >
                      {item.name}
                    </CardTitle>
                  </Tooltip>
                  ...
                  <div className="flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        ...
                        onClick={() => {
                          ...
                          toastSuccess(
                            item.status === "active"
                              ...
                              : "Delivery man inactivated successfully"
                          );
                        }}
                      >
                        {item.status === "active" ? (
                    ...
                    >
                      <div
                        ...
                        onClick={() => {
                          ...
                          toastSuccess(
                            item.isDeleted
                              ? "Delivery man restored successfully"
                    ...
                    <Tooltip label="Edit" position="top" withArrow>
                      <div
                        ...
                        onClick={() => navigate(`/deliverymen/edit/1`)}
                      >
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">Loading more delivery men...</span>
              </div>
            </div>
          )}
      ...
      <Modal
        ...
        title={
          <div className="">
            <h3 className="text-lg font-semibold pl-4">Import Deliverymen</h3>
          </div>
        }
        size="xl"
        overlayProps={{
