...
import { mockDeliverymenHistoryData } from "@/mockData/delivary-men-history-mockdata";
...
const MOCK_TABLE_ASSIGN_DATA = MOCK_TABLE_ASSIGN.map((item) => item.name);

export default function DeliverymenDetails() {
  const { t } = useTranslation();
  const navigate = useNavigate();

  const [keepChanges, setKeepChanges] = useState(false);
  const [isOptionModalOpen, setIsOptionModalOpen] = useState(false);
  const [selectedReservationNo, setSelectedReservationNo] = useState("Nike");
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const blob = await pdf(
        <PDF
          data={[viewData]}
          title="Deliverymen Details"
          subtitle="Deliverymen Information Report"
        />
      ).toBlob();
  ...
  return (
    <>
      <PageLayout
        title={t("button.viewingDeliverymen")}
        videoSrc={video}
        videoHeader="Rapid ERP Video"
        onListClick={() => navigate("/deliverymen")}
        listText="List"
        listPath="/deliverymen"
        popoverOptions={[
          {
            label: "Create",
            onClick: () => navigate("/deliverymen/create"),
          },
          {
            label: "Edit",
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div className="md:col-span-3">
            <h3 className="font-medium mb-1">Deliverymen Name</h3>
          ...
          <div className="md:col-span-3">
            <h3 className="font-medium mb-1">National ID</h3>
            <div className="w-full px-1 py-1 text-gray-500 font-normal text-md">
              {viewData.nationalId}
            </div>
          </div>
          <div className="md:col-span-3">
            <h3 className="font-medium mb-1">Vehicle Type</h3>
            <div className="w-full px-1 py-1 text-gray-500 font-normal text-md">
              {viewData.vehicleType}
            </div>
          </div>
          <div className="md:col-span-3">
            <h3 className="font-medium mb-1">Assigned Area</h3>
            <div className="w-full px-1 py-1 text-gray-500 font-normal text-md">
              {viewData.assignedArea}
            </div>
          </div>
        </div>
      ...
      >
        ...
        <Modal.Body>
          <HistoryDataTable columnData={mockDeliverymenHistoryData} />
        </Modal.Body>
      </Modal>
    </>
  );
