...
export default function CourierCreatePage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("courier", "view");
  const canEdit = usePermission("courier", "edit");
  const canDelete = usePermission("courier", "delete");
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/courier/create");
        } else {
          navigate("/courier/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Courier created successfully!");
      handleReset();
    } else {
      toastSuccess("Courier created successfully!");
      navigate("/courier");
  ...
  const handleReset = () => {
    ...
    setTimeout(() => {
      inputRefs.current["documentRequired"]?.focus();
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Courier saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Courier" : "Creating Courier"}
        videoSrc={video}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              {permissionsFieldLevel.documentRequired && (
                <div className="space-y-2">
                  <Autocomplete
                    ref={(el: any) => setRef("documentRequired")(el)}
                    id="documentRequired"
              ...
              {permissionsFieldLevel.courierName && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    tooltipText="Enter courier name"
                    required
                  />
                </div>
              )}
              ...
              {permissionsFieldLevel.contactPerson && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    onChange={handleChange}
                    onNext={() => focusNextInput("airwayLocation")}
                    onCancel={() =>
                      setFormData({ ...formData, airwayLocation: "" })
                    }
                    labelText="Contact Person"
                    tooltipText="Enter contact person"
                    required
                  />
                </div>
              )}
              ...
              {permissionsFieldLevel.airwayLocation && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("airwayLocation")}
                    type="text"
                    id="airwayLocation"
                    name="airwayLocation"
                    value={formData.airwayLocation}
                    onChange={handleChange}
                    onNext={() => focusNextInput("trackingNo")}
                    onCancel={() =>
                      setFormData({ ...formData, airwayLocation: "" })
                    }
                    labelText="Airway Location"
                    tooltipText="Enter airway location"
                    required
                  />
                </div>
              )}
              ...
              {permissionsFieldLevel.trackingNo && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    onCancel={() =>
                      setFormData({ ...formData, trackingNo: "" })
                    }
                    labelText="Tracking No"
                    tooltipText="Enter Tracking No"
                    required
                  />
                </div>
              )}
