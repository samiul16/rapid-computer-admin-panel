...
export default function DependantsDetails() {
  ...
  // Get dependants data based on selected employee
  const getDependantsData = (employeeName: string): DetailsPageTypes => {
    const dependantsMap: Record<string, DetailsPageTypes> = {
      "John Smith": initialData,
      "Maria Garcia": initialData,
      "David Johnson": initialData,
      "Lisa Chen": initialData,
      "Michael Brown": initialData,
      "Anna Wilson": initialData,
      "Robert Taylor": initialData,
      "Jennifer Davis": initialData,
      "Christopher Lee": initialData,
      "Sarah Martinez": initialData,
      "William Anderson": initialData,
      "Jessica White": initialData,
    };

    return dependantsMap[employeeName] || initialData;
  };

  const [dependantsData, setDependantsData] = useState<DetailsPageTypes>(
    getDependantsData(selectedEmployee)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update dependants data when selection changes
  useEffect(() => {
    setDependantsData(getDependantsData(selectedEmployee));
  }, [selectedEmployee]);
  ...
  const handlePrintDependants = (dependantsData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", dependantsData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintDependants(dependantsData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Employee Selection, Dependants Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.employeeName && (
              <div className="mt-1">
                <Autocomplete
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((dependantsData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(dependantsData.createdAt),
          updated: getRelativeTime(dependantsData.updatedAt),
          drafted: getRelativeTime(dependantsData.draftedAt),
          deleted: getRelativeTime(dependantsData.deletedAt),
        }}
      />
