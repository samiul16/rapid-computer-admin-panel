...
export default function ScheduleDetails() {
  ...
  // Get schedule data based on selected schedule
  const getScheduleData = (scheduleName: string): DetailsPageTypes => {
    const scheduleMap: Record<string, DetailsPageTypes> = {
      "Dr. John Doe": initialData,
      "Dr. Jane Smith": initialData,
      "Dr. Ahmed Khan": initialData,
      "Dr. Liu Wen": initialData,
      "Dr. Carlos Diaz": initialData,
      "Dr. Sarah Johnson": initialData,
      "Dr. Michael Brown": initialData,
      "Dr. Emily Davis": initialData,
      "Dr. David Wilson": initialData,
      "Dr. Lisa Anderson": initialData,
      "Dr. Robert Taylor": initialData,
      "Dr. Maria Garcia": initialData,
    };

    return scheduleMap[scheduleName] || initialData;
  };

  const [scheduleData, setScheduleData] = useState<DetailsPageTypes>(
    getScheduleData(selectedSchedule)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update schedule data when selection changes
  useEffect(() => {
    setScheduleData(getScheduleData(selectedSchedule));
  }, [selectedSchedule]);
  ...
  const handlePrintSchedule = (scheduleData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", scheduleData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintSchedule(scheduleData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Schedule Selection, Schedule Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.doctorName && (
              <div className="mt-1">
                <Autocomplete
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((scheduleData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(scheduleData.createdAt),
          updated: getRelativeTime(scheduleData.updatedAt),
          drafted: getRelativeTime(scheduleData.draftedAt),
          deleted: getRelativeTime(scheduleData.deletedAt),
        }}
      />
