import { Card, CardTitle } from "@/components/ui/card";
import { toastDelete, toastRestore } from "@/lib/toast";
import { Tooltip } from "@mantine/core"; // Import Tooltip from Mantine
import { RefreshCw, Trash2 } from "lucide-react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faEdit } from "@fortawesome/free-solid-svg-icons";
import { useCallback, useEffect, useRef, useState } from "react";
import { cn } from "@/lib/utils";
import { useNavigate } from "react-router-dom";
import GridExportComponent from "./GridExportComponent";
import GridFilterComponent from "./GridFilterComponent";
import { usePermission } from "@/hooks/usePermissions";

type GridDataType = {
  vehicle: string;
  vin: string;
  licensePlate: string;
  vehicleType: string;
  year: string;
  make: string;
  model: string;
  trim: string;
  registration: string;
  vehicleGroup: string;
  ownership: string;
  color: string;
  bodyType: string;
  msrp: string;

  id: string;
  isDefault: boolean;
  isActive: boolean;
  isDraft: boolean;
  createdAt: Date;
  draftedAt: Date | null;
  updatedAt: Date;
  deletedAt: Date | null;
  isDeleted: boolean;
};

const vehicleData: GridDataType[] = [
  {
    vehicle: "Toyota Corolla",
    vin: "1HGCM82633A123456",
    licensePlate: "DHA-1234",
    vehicleType: "Sedan",
    year: "2020",
    make: "Toyota",
    model: "Corolla",
    trim: "LE",
    registration: "REG202001",
    vehicleGroup: "Passenger",
    ownership: "Owned",
    color: "White",
    bodyType: "Compact",
    msrp: "$20,000",
    id: "1",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-01-01"),
    draftedAt: null,
    updatedAt: new Date("2023-05-01"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Honda Civic",
    vin: "2HGFA16598H123457",
    licensePlate: "CTG-5678",
    vehicleType: "Sedan",
    year: "2021",
    make: "Honda",
    model: "Civic",
    trim: "EX",
    registration: "REG202102",
    vehicleGroup: "Passenger",
    ownership: "Leased",
    color: "Black",
    bodyType: "Compact",
    msrp: "$22,500",
    id: "2",
    isDefault: true,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-02-15"),
    draftedAt: null,
    updatedAt: new Date("2023-06-01"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Ford F-150",
    vin: "1FTFW1EF1EKF12345",
    licensePlate: "SYL-4567",
    vehicleType: "Truck",
    year: "2019",
    make: "Ford",
    model: "F-150",
    trim: "XLT",
    registration: "REG201903",
    vehicleGroup: "Commercial",
    ownership: "Owned",
    color: "Blue",
    bodyType: "Pickup",
    msrp: "$35,000",
    id: "3",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-03-10"),
    draftedAt: null,
    updatedAt: new Date("2023-07-12"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "BMW X5",
    vin: "5UXKR0C54E0J12345",
    licensePlate: "KHL-8901",
    vehicleType: "SUV",
    year: "2022",
    make: "BMW",
    model: "X5",
    trim: "xDrive40i",
    registration: "REG202204",
    vehicleGroup: "Passenger",
    ownership: "Owned",
    color: "Silver",
    bodyType: "SUV",
    msrp: "$60,000",
    id: "4",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-04-20"),
    draftedAt: null,
    updatedAt: new Date("2023-08-05"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Nissan Altima",
    vin: "1N4AL3AP3JC123456",
    licensePlate: "RAJ-2345",
    vehicleType: "Sedan",
    year: "2020",
    make: "Nissan",
    model: "Altima",
    trim: "SL",
    registration: "REG202005",
    vehicleGroup: "Passenger",
    ownership: "Leased",
    color: "Red",
    bodyType: "Mid-Size",
    msrp: "$25,000",
    id: "5",
    isDefault: false,
    isActive: false,
    isDraft: true,
    createdAt: new Date("2023-05-15"),
    draftedAt: new Date("2023-06-01"),
    updatedAt: new Date("2023-06-10"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Chevrolet Tahoe",
    vin: "1GNSKCKC0GR123456",
    licensePlate: "BRS-3456",
    vehicleType: "SUV",
    year: "2021",
    make: "Chevrolet",
    model: "Tahoe",
    trim: "Premier",
    registration: "REG202106",
    vehicleGroup: "Commercial",
    ownership: "Owned",
    color: "Black",
    bodyType: "SUV",
    msrp: "$55,000",
    id: "6",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-06-25"),
    draftedAt: null,
    updatedAt: new Date("2023-09-01"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Hyundai Sonata",
    vin: "5NPE24AF0FH123456",
    licensePlate: "MYM-4567",
    vehicleType: "Sedan",
    year: "2019",
    make: "Hyundai",
    model: "Sonata",
    trim: "SE",
    registration: "REG201907",
    vehicleGroup: "Passenger",
    ownership: "Leased",
    color: "White",
    bodyType: "Mid-Size",
    msrp: "$19,500",
    id: "7",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-07-10"),
    draftedAt: null,
    updatedAt: new Date("2023-10-02"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Tesla Model 3",
    vin: "5YJ3E1EA7KF123456",
    licensePlate: "CUM-6789",
    vehicleType: "Sedan",
    year: "2022",
    make: "Tesla",
    model: "Model 3",
    trim: "Long Range",
    registration: "REG202208",
    vehicleGroup: "Passenger",
    ownership: "Owned",
    color: "White",
    bodyType: "Electric",
    msrp: "$48,000",
    id: "8",
    isDefault: true,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-08-15"),
    draftedAt: null,
    updatedAt: new Date("2023-11-01"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Kia Sportage",
    vin: "KNDPNCAC7L7123456",
    licensePlate: "GOP-7890",
    vehicleType: "SUV",
    year: "2020",
    make: "Kia",
    model: "Sportage",
    trim: "LX",
    registration: "REG202009",
    vehicleGroup: "Passenger",
    ownership: "Owned",
    color: "Grey",
    bodyType: "SUV",
    msrp: "$27,000",
    id: "9",
    isDefault: false,
    isActive: false,
    isDraft: true,
    createdAt: new Date("2023-09-12"),
    draftedAt: new Date("2023-09-15"),
    updatedAt: new Date("2023-09-20"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Audi Q7",
    vin: "WA1LAAF71KD123456",
    licensePlate: "NGA-8901",
    vehicleType: "SUV",
    year: "2021",
    make: "Audi",
    model: "Q7",
    trim: "Premium Plus",
    registration: "REG202110",
    vehicleGroup: "Passenger",
    ownership: "Owned",
    color: "Black",
    bodyType: "SUV",
    msrp: "$65,000",
    id: "10",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-10-05"),
    draftedAt: null,
    updatedAt: new Date("2024-01-01"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Mercedes-Benz C-Class",
    vin: "WDDWF4JB6HR123456",
    licensePlate: "BGR-9012",
    vehicleType: "Sedan",
    year: "2022",
    make: "Mercedes-Benz",
    model: "C300",
    trim: "4MATIC",
    registration: "REG202211",
    vehicleGroup: "Passenger",
    ownership: "Leased",
    color: "Silver",
    bodyType: "Luxury",
    msrp: "$50,000",
    id: "11",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-11-20"),
    draftedAt: null,
    updatedAt: new Date("2024-02-01"),
    deletedAt: null,
    isDeleted: false,
  },
  {
    vehicle: "Toyota Hilux",
    vin: "MR0DZ22G102123456",
    licensePlate: "RNG-1122",
    vehicleType: "Truck",
    year: "2020",
    make: "Toyota",
    model: "Hilux",
    trim: "Revo",
    registration: "REG202012",
    vehicleGroup: "Commercial",
    ownership: "Owned",
    color: "White",
    bodyType: "Pickup",
    msrp: "$40,000",
    id: "12",
    isDefault: false,
    isActive: true,
    isDraft: false,
    createdAt: new Date("2023-12-15"),
    draftedAt: null,
    updatedAt: new Date("2024-03-01"),
    deletedAt: null,
    isDeleted: false,
  },
];

type Props = {
  searchQuery: string;
  setIsFilterOpen: (isFilterOpen: boolean) => void;
  isFilterOpen: boolean;
  setIsExportOpen: (isExportOpen: boolean) => void;
  isExportOpen: boolean;
};

export default function PropertyApprovalGrid({
  searchQuery,
  setIsFilterOpen,
  isFilterOpen,
  setIsExportOpen,
  isExportOpen,
}: Props) {
  console.log("grid rendered");

  const navigate = useNavigate();

  const [gridData, setGridData] = useState(vehicleData);
  const canDelete: boolean = usePermission("vehicles", "delete");
  const canRestore: boolean = usePermission("vehicles", "restore");
  const canEdit: boolean = usePermission("vehicles", "edit");

  // Debug permissions
  console.log("Vehicles Permissions:", {
    canDelete,
    canRestore,
    canEdit,
  });

  // Infinite scroll states
  const [isLoading, setIsLoading] = useState(false);
  const [hasMore, setHasMore] = useState(true);
  const [, setPage] = useState(1);
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const ITEMS_PER_PAGE = 4;

  // Simulate API call to load more data
  const loadMoreData = useCallback(async () => {
    if (isLoading || !hasMore) return;

    setIsLoading(true);

    await new Promise((resolve) => setTimeout(resolve, 800));

    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      vehicle: `Vehicle ${index + 1}`,
      vin: `VIN ${index + 1}`,
      licensePlate: `License Plate ${index + 1}`,
      vehicleType: `Vehicle Type ${index + 1}`,
      year: `Year ${index + 1}`,
      make: `Make ${index + 1}`,
      model: `Model ${index + 1}`,
      trim: `Trim ${index + 1}`,
      registration: `Registration ${index + 1}`,
      vehicleGroup: `Vehicle Group ${index + 1}`,
      ownership: `Ownership ${index + 1}`,
      color: `Color ${index + 1}`,
      bodyType: `Body Type ${index + 1}`,
      msrp: `MSRP ${index + 1}`,

      isDefault: false,
      isActive: Math.random() > 0.3,
      isDraft: Math.random() > 0.7,
      createdAt: new Date(),
      draftedAt: null,
      updatedAt: new Date(),
      deletedAt: null,
      isDeleted: false,
    }));

    // Stop loading more after reaching 50 items for demo
    if (gridData.length >= 46) {
      setHasMore(false);
    } else {
      setGridData((prev) => [...prev, ...newItems] as GridDataType[]);
      setPage((prev) => prev + 1);
    }

    setIsLoading(false);
  }, [gridData.length, isLoading, hasMore]);

  // Infinite scroll handler
  const handleScroll = useCallback(() => {
    const container = scrollContainerRef.current;
    if (!container) return;

    const { scrollTop, scrollHeight, clientHeight } = container;
    const threshold = 100; // Load more when 100px from bottom

    if (scrollHeight - scrollTop <= clientHeight + threshold) {
      loadMoreData();
    }
  }, [loadMoreData]);

  // Add scroll event listener
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container) return;

    container.addEventListener("scroll", handleScroll);
    return () => container.removeEventListener("scroll", handleScroll);
  }, [handleScroll]);

  const handleDeleteClick = (id: string) => {
    setGridData((prev) =>
      prev.map((item) =>
        item.id === id
          ? {
              ...item,
              isDeleted: item.isDeleted === true ? false : true,
            }
          : item
      )
    );
  };

  const handleRestoreClick = (id: string) => {
    setGridData((prev) =>
      prev.map((item) =>
        item.id === id
          ? {
              ...item,
              isDeleted: item.isDeleted === true ? false : true,
            }
          : item
      )
    );
  };

  // Filter leaves based on search query
  const filteredData = gridData.filter(
    (item) =>
      item.vehicle.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.vin.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.licensePlate.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.vehicleType.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.year.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.make.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.model.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.trim.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.registration.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.vehicleGroup.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.ownership.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.color.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.bodyType.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.msrp.toLowerCase().includes(searchQuery.toLowerCase())
  );

  return (
    <div
      className={cn(
        "px-4 py-3 h-full flex flex-col bg-white dark:bg-gray-900 parent relative rounded-lg"
      )}
    >
      {/* Floating Label - Left Top */}
      <div
        className={cn(
          "absolute -top-4 left-6 rtl:left-auto rtl:right-6 py-1 rounded-md z-40! bg-white w-fit"
        )}
      >
        <span
          className={cn(
            "text-md font-semibold tracking-wide capitalize text-gray-600"
          )}
        >
          Total {gridData.length} Vehicles
        </span>
      </div>

      {/* Main content area */}
      <div className="flex flex-1 overflow-hidden mt-2">
        {/* Cards container */}
        <div
          ref={scrollContainerRef}
          className="overflow-y-auto scroll-smooth smooth-scroll pr-4"
          style={{
            width: isFilterOpen || isExportOpen ? "calc(100% - 320px)" : "100%",
          }}
        >
          <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 pb-4 p-2">
            {filteredData.map((item, index) => (
              <Card
                key={index}
                className="transition-all hover:border-primary/90 hover:shadow-lg hover:translate-y-[-5px] relative group dark:bg-gray-800 p-4 duration-200"
              >
                {/* Top Row - Grid with 2 columns: Title | Status */}
                <div className="grid grid-cols-2 items-center gap-2 mb-4">
                  {/* Left - Title */}
                  <CardTitle
                    className="text-lg font-semibold cursor-pointer hover:text-primary transition-colors truncate"
                    onClick={() => navigate(`/vehicles/1`)}
                  >
                    {item.vehicle}
                  </CardTitle>

                  {/* Right - Status */}
                  <div className="flex items-end flex-col">
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      license plate
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                      {item.licensePlate}
                    </div>
                  </div>
                </div>

                {/* Bottom Row - Grid with 3 columns: Leave Type | Actions | Notes */}
                <div className="grid grid-cols-2 items-center gap-4 pt-2 dark:border-gray-700">
                  {/* Leave Type - Left aligned */}
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      vin
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                      {item.vin}
                    </div>
                  </div>

                  {/* Middle - Action Icons */}
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    {/* Delete/Restore */}
                    <Tooltip
                      label={
                        item.isDeleted && canRestore
                          ? "Restore"
                          : canDelete
                          ? "Delete"
                          : ""
                      }
                      position="top"
                      arrowSize={8}
                      withArrow
                      styles={{
                        tooltip: {
                          fontSize: "14px",
                          padding: "8px 12px",
                          backgroundColor: "#374151",
                          color: "white",
                          borderRadius: "6px",
                          fontWeight: "500",
                          boxShadow:
                            "0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)",
                        },
                        arrow: {
                          backgroundColor: "#374151",
                        },
                      }}
                    >
                      <button
                        disabled={item.isDeleted && !canRestore}
                        className={`cursor-pointer p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${
                          item.isDeleted ? "text-blue-500" : "text-red-500"
                        }`}
                        onClick={() => {
                          if (canRestore && item.isDeleted) {
                            handleRestoreClick(item.id);
                            toastRestore("Vehicle restored successfully");
                          } else {
                            if (canDelete) {
                              handleDeleteClick(item.id);
                              toastDelete("Vehicle deleted successfully");
                            }
                          }
                        }}
                      >
                        {item.isDeleted && canRestore ? (
                          <RefreshCw className="h-4 w-4" />
                        ) : (
                          canDelete && <Trash2 className="h-4 w-4" />
                        )}
                      </button>
                    </Tooltip>

                    {/* Edit */}
                    {canEdit && (
                      <Tooltip
                        label="Edit"
                        position="top"
                        arrowSize={8}
                        withArrow
                        styles={{
                          tooltip: {
                            fontSize: "14px",
                            padding: "8px 12px",
                            backgroundColor: "#374151",
                            color: "white",
                            borderRadius: "6px",
                            fontWeight: "500",
                            boxShadow:
                              "0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)",
                          },
                          arrow: {
                            backgroundColor: "#374151",
                          },
                        }}
                      >
                        <div
                          className="cursor-pointer p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-blue-500 flex items-center justify-center w-8 h-8"
                          onClick={() => navigate(`/vehicles/edit/1`)}
                        >
                          <FontAwesomeIcon icon={faEdit} className="h-4 w-4" />
                        </div>
                      </Tooltip>
                    )}
                  </div>
                </div>

                <div className="grid grid-cols-3 items-center gap-4 pt-2 dark:border-gray-700 border-t">
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      vehicle type
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                      {item.vehicleType}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      year
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                      {item.year}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      make
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                      {item.make}
                    </div>
                  </div>
                </div>
              </Card>
            ))}
          </div>

          {/* Loading indicator */}
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
                <span className="text-sm">Loading more vehicles...</span>
              </div>
            </div>
          )}

          {/* End of data indicator */}
          {!hasMore && gridData.length > 12 && (
            <div className="flex justify-center items-center py-8">
              <span className="text-sm text-gray-500 dark:text-gray-400">
                No more vehicles to load
              </span>
            </div>
          )}
        </div>

        {/* Filter component - Right side only */}
        {isFilterOpen && (
          <div className="w-80 flex-shrink-0 border-l border-gray-200 dark:border-gray-700 pl-4">
            <div className="h-full flex flex-col">
              <GridFilterComponent
                data={gridData}
                setFilteredData={setGridData}
                setShowFilter={setIsFilterOpen}
              />
            </div>
          </div>
        )}

        {/* Export component - Right side only */}
        {isExportOpen && (
          <div className="w-80 flex-shrink-0 border-l border-gray-200 dark:border-gray-700 pl-4">
            <div className="h-full flex flex-col">
              <GridExportComponent
                data={gridData}
                setFilteredData={setGridData}
                setIsExportOpen={setIsExportOpen}
              />
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
