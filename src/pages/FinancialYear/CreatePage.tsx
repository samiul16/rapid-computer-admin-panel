...
const initialData: FinancialYearData = {
  title: "",
  fromDate: "",
  toDate: "",
  status: "",
...
export default function FinancialYearCreatePage({ isEdit = false }: Props) {
  ...
  const title: boolean = usePermission("financialYear", "create", "name");
  ...
  const isDraft: boolean = usePermission("financialYear", "create", "isDraft");
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/financial-year/create");
        } else {
          navigate("/financial-year/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Financial Year created successfully!");
      handleReset();
    } else {
      toastSuccess("Financial Year created successfully!");
  ...
  const handlePrintIncrements = (FinancialYearData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("FinancialYearData on pdf click", formData);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Financial Year saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={
          isEdit ? labels.editingFinancialYear : labels.creatingFinancialYear
        }
        videoSrc={video}
              ...
              {canCreate && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    ...
                    labelText={labels.fromDateTooltip}
                    tooltipText={labels.fromDateTooltip}
                    required
                    type="date"
                  />
                </div>
              ...
              {canCreate && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    ...
                    labelText={labels.toDateTooltip}
                    tooltipText={labels.toDateTooltip}
                    required
                    type="date"
                  />
                </div>
              ...
              {canCreate && (
                <div className="md:col-span-3 space-y-2 relative">
                  <Autocomplete
                    ...
                    value={formData.status === "Yes" ? labels.yes : labels.no}
                    labelClassName="rounded-lg"
                    isSelectableOnly={true}
                    onValueChange={(value: string) => {
                      const isYes = value === labels.yes;
                      const newValue = isYes ? "Yes" : "No";
                      setFormData((prev) => ({
                        ...prev,
                        status: newValue,
                      }));
