...
export default function AllowanceTypeDetails() {
  ...
  // Get allowance type data based on selected allowance type
  const getAllowanceTypeData = (
    allowanceTypeName: string
  ): DetailsPageTypes => {
    const allowanceTypeMap: Record<string, DetailsPageTypes> = {
      "Housing Allowance": initialData,
      "Transportation Allowance": initialData,
      "Meal Allowance": initialData,
      "Medical Allowance": initialData,
      "Education Allowance": initialData,
      "Communication Allowance": initialData,
    };

    return allowanceTypeMap[allowanceTypeName] || initialData;
  };

  const [allowanceTypeData, setAllowanceTypeData] = useState<DetailsPageTypes>(
    getAllowanceTypeData(selectedAllowanceType)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update allowance type data when selection changes
  useEffect(() => {
    setAllowanceTypeData(getAllowanceTypeData(selectedAllowanceType));
  ...
  const handlePrintAllowanceType = (allowanceTypeData: any) => {
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          ...
          isDefault: "Default Allowance Type",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", allowanceTypeData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintAllowanceType(allowanceTypeData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Allowance Type Selection, Allowance Type Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.allowanceType && (
              <div className="mt-1">
                <Autocomplete
                  options={autocompleteDataDetailsPage}
                  value={selectedAllowanceType}
                  onValueChange={setSelectedAllowanceType}
                  placeholder=" "
                  displayKey="name"
                  valueKey="name"
                  searchKey="name"
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((allowanceTypeData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(allowanceTypeData.createdAt),
          updated: getRelativeTime(allowanceTypeData.updatedAt),
          drafted: getRelativeTime(allowanceTypeData.draftedAt),
          deleted: getRelativeTime(allowanceTypeData.deletedAt),
        }}
      />
