...
const mockTaxRates = [
  ...
  {
    id: "8",
    name: "Digital Services Tax Rate",
    taxRate: 14,
  ...
  {
    id: "9",
    name: "Health and Wellness Tax Rate",
    taxRate: 7,
  ...
  {
    id: "10",
    name: "Tourism Development Tax Rate",
    taxRate: 9,
...
export default function TaxRatesDataTable2({
  viewMode,
  setViewMode,
  dataTableFilter,
  searchQuery,
  ...
  const componentColumns = [
    {
      accessorKey: "name",
      title: "Name",
      options: [...new Set(mockTaxRates.map((item) => item.name))],
...

    {
      accessorKey: "taxRate",
      title: "Tax Rate(%)",
      options: [...new Set(mockTaxRates.map((item) => item.taxRate))],
      filterFn: (row: any, columnId: any, filterValue: any) => {
        if (!filterValue || filterValue.length === 0) return true;
        const cellValue = row.getValue(columnId) as string;
        return filterValue.some((filterVal: string) =>
      ...
      sortingFn: (row1: any, row2: any) => {
        return row1.getValue("taxRate").localeCompare(row2.getValue("taxRate"));
      },
      size: 180,
      minSize: 150,
      meta: {
  ...
  const filteredData = mockTaxRates.filter((finance) => {
