...
export default function TaxRatesEditPage({ isEdit = false }: Props) {
  ...
  const isDefault: boolean = usePermission("taxRates", "create", "isDefault");
  const isDraft: boolean = usePermission("taxRates", "create", "isDraft");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toastSuccess("TaxRates Edited successfully");
  };
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/tax-rates/create");
        } else {
          navigate("/tax-rates/edit/undefined");
        }
      },
    },
    {
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("TaxRates saved as draft successfully");
            },
          },
        ];
      }
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? labels.editingTexRates : labels.creatingTexRates}
        videoSrc={video}
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 my-8">
              ...
              {canEdit && (
                <div className="md:col-span-3 space-y-2">
                  <EditableInput
                    setRef={setRef("taxRate")}
                    id="taxRate"
                    name="taxRate"
                    value={formData.taxRate ? formData.taxRate.toString() : ""}
                    onChange={handleChange}
                    onNext={() => focusNextInput("isDefault")}
                    onCancel={() => setFormData({ ...formData, taxRate: 0 })}
                    labelText={labels.taxRateTooltip}
                    tooltipText={labels.taxRateTooltip}
                    required
                    type="number"
                  />
                </div>
