...
export default function PromocodesDetails() {
  ...
  // Get promocodes data based on selected promocode
  const getPromocodesData = (promocodeName: string): DetailsPageTypes => {
    const promocodesMap: Record<string, DetailsPageTypes> = {
      "Standard Room": initialData,
      "Deluxe Room": initialData,
      Suite: initialData,
      "Family Room": initialData,
      "Executive Room": initialData,
      "Presidential Suite": initialData,
      Penthouse: initialData,
      "Ocean View Room": initialData,
      "Garden Room": initialData,
      "Business Room": initialData,
      "Luxury Suite": initialData,
      "VIP Room": initialData,
    };

    return promocodesMap[promocodeName] || initialData;
  };

  const [promocodesData, setPromocodesData] = useState<DetailsPageTypes>(
    getPromocodesData(selectedPromocode)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update promocodes data when selection changes
  useEffect(() => {
    setPromocodesData(getPromocodesData(selectedPromocode));
  }, [selectedPromocode]);
  ...
  const handlePrintPromocodes = (promocodesData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", promocodesData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintPromocodes(promocodesData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Promocode Selection, Promocode Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.roomSizeName && (
              <div className="mt-1">
                <Autocomplete
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((promocodesData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(promocodesData.createdAt),
          updated: getRelativeTime(promocodesData.updatedAt),
          drafted: getRelativeTime(promocodesData.draftedAt),
          deleted: getRelativeTime(promocodesData.deletedAt),
        }}
      />
