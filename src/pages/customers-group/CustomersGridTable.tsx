...
type CustomerGroup = {
  ...
  customerLogo: string; // URL to logo image
...
const customers: CustomerGroup[] = [
  {
    ...
    currency: "USD",
    customerLogo: "/customer-dummy-image.jpg",
    status: "active",
    isDeleted: false,
  },
  {
  ...
  },
  {
    id: "5",
    customerNo: "C005",
    customerName: "London Supplies Ltd",
    paymentMode: "Bank Transfer",
    currency: "GBP",
    customerLogo: "/customer-dummy-image.jpg",
    status: "active",
...
export default function CustomersGridTable({
  setViewMode,
}: {
  setViewMode: (viewMode: "grid" | "list") => void;
}) {
  console.log("Customers grid rendered");
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      customerNo: `C${(customersData.length + index + 1)
        .toString()
        .padStart(2, "0")}`,
      customerName: `${customersData.length + index + 1} Customer`,
    ...
    if (customersData.length >= 46) {
      setHasMore(false);
    } else {
      setCustomersData((prev) => [...prev, ...(newItems as CustomerGroup[])]);
      setPage((prev) => prev + 1);
    }

    setIsLoading(false);
  }, [customersData.length, isLoading, hasMore]);
    ...
    setCustomersData((prevCustomers) =>
      prevCustomers.map((customer) =>
        customer.id === customerId
          ? {
              ...customer,
              isDeleted: customer.isDeleted === true ? false : true,
            }
          : customer
      )
    );
  ...
  const handleRestoreClick = (customerId: string) => {
    setCustomersData((prevCustomers) =>
      prevCustomers.map((customer) =>
        customer.id === customerId
          ? {
  ...
  const filteredCustomers = customersData.filter(
    (customer) =>
      ...
      customer.paymentMode.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="px-4 py-3 h-full flex flex-col bg-white dark:bg-gray-900 parent">
      ...
      <div className="flex flex-1 overflow-hidden mt-2">
        ...
        >
              ...
              >
                ...
                <div className="grid grid-cols-3 items-center gap-2 mb-4">
                  ...
                  <div className="flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    {/* Delete/Restore */}
                    <Tooltip
                      label={customer.isDeleted ? "Restore" : "Delete"}
                      position="top"
                      withArrow
                    >
                      <div
                        className={`cursor-pointer p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${
                          customer.isDeleted ? "text-blue-500" : "text-red-500"
                        }`}
                        onClick={() => {
                          if (customer.isDeleted) {
                            handleRestoreClick(customer.id);
                          } else {
                  ...
                  <div className="flex justify-end">
                    <img
                      src={customer.customerLogo.toLowerCase()}
                      alt={`${customer.customerName} flag`}
          ...
          {!hasMore && filteredCustomers.length > 12 && (
            <div className="flex justify-center items-center py-8">
        ...
        {isFilterOpen && (
          <div className="w-80 flex-shrink-0 border-l border-gray-200 dark:border-gray-700 pl-4">
            <div className="h-full flex flex-col">
              <GridFilterComponent
                data={customers}
                setFilteredData={setCustomersData}
      ...
      <Modal
        ...
        title={
          <div className="">
            <h3 className="text-lg font-semibold pl-4 ">Import Customer</h3>
          </div>
        }
        size="xl"
        overlayProps={{
