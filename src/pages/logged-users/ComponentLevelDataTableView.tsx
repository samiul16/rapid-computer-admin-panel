...
export default function ComponentLevelDataTableView({
  ...
  // Create custom pathName for Google Maps navigation
  const createGoogleMapsPathName = () => {
    // Check if we have any data with coordinates
    const hasCoordinates = filteredData.some(
      (item) => item.latitude && item.longitude
    );
    if (hasCoordinates) {
      return `google-maps:${location.pathname.split("/")[1]}`;
    }
  ...
  return (
    <FixedColumnDataTable
      ...
      pathName={createGoogleMapsPathName()}
