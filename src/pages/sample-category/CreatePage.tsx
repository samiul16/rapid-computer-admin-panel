...
export default function SampleCategoryFormPage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("sampleCategory", "view");
  const canEdit = usePermission("sampleCategory", "edit");
  const canDelete = usePermission("sampleCategory", "delete");
  ...
  const name: boolean = usePermission("sampleCategory", "create", "name");
  ...
  const isDraft: boolean = usePermission("sampleCategory", "create", "isDraft");
  ...
  // Form state
  const [formData, setFormData] = useState<SampleCategoryData>({
    name: "",
    description: "",
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/sample-category/create");
        } else {
          navigate("/sample-category/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (printEnabled) {
      handlePrintSampleCategory(formData);
    }

    // keep switch functionality
    if (keepCreating) {
      toastSuccess("Sample category created successfully!");
      handleReset();
    } else {
      toastSuccess("Sample category created successfully!");
      navigate("/sample-category");
    }
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("sampleCategoryData on pdf click", formData);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Sample category saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Sample Category" : "Creating Sample Category"}
        videoSrc={video}
              ...
              {description && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    tooltipText="Enter sample category description"
                    required
                  />
                </div>
              )}
