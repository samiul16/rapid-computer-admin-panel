...
export default function PaymentModeDetails() {
  ...
  // Get payment mode data based on selected payment mode
  const getPaymentModeData = (paymentModeName: string): DetailsPageTypes => {
    const paymentModeMap: Record<string, DetailsPageTypes> = {
      "Cash Payment": initialData,
      "Bank Transfer": initialData,
      "Credit Card": initialData,
      "Mobile Payment": initialData,
      "Cheque Payment": initialData,
      "Online Payment": initialData,
    };

    return paymentModeMap[paymentModeName] || initialData;
  };

  const [paymentModeData, setPaymentModeData] = useState<DetailsPageTypes>(
    getPaymentModeData(selectedPaymentMode)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update payment mode data when selection changes
  useEffect(() => {
    setPaymentModeData(getPaymentModeData(selectedPaymentMode));
  }, [selectedPaymentMode]);
  ...
  const handlePrintPaymentMode = (paymentModeData: any) => {
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          ...
          isDefault: "Default Payment Mode",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", paymentModeData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintPaymentMode(paymentModeData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Payment Mode Selection, Payment Mode Details */}
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((paymentModeData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(paymentModeData.createdAt),
          updated: getRelativeTime(paymentModeData.updatedAt),
          drafted: getRelativeTime(paymentModeData.draftedAt),
          deleted: getRelativeTime(paymentModeData.deletedAt),
        }}
      />
