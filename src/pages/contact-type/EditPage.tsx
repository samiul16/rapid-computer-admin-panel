...
const initialData: ContactTypeData = {
  name: "Sample Contact Type",
  description: "Sample description for contact type",
  isDraft: false,
  createdAt: new Date(),
  draftedAt: null,
  updatedAt: new Date(),
...
export default function ContactTypeEditPage({ isEdit = false }: Props) {
  ...
  // No interviewer options for contact type

  // File upload states (removed for bonus)

  // Form state
  const [formData, setFormData] = useState<ContactTypeData>({
    name: "",
    description: "",
    isDraft: false,
    isDeleted: false,
  ...
  const canView = usePermission("contactType", "view");
  const canEdit = usePermission("contactType", "edit");

  // Field-level permissions
  const namePerm: boolean = usePermission("contactType", "edit", "name");
  ...
  // No iqama/branch/from/to for contact type edit
  ...
  // no auto-calculation needed for contact type
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (printEnabled) {
      handlePrintContactType(formData);
    }

    // keep switch functionality
    if (keepCreating) {
      toastSuccess("Contact Type updated successfully!");
      handleReset();
    } else {
      toastSuccess("Contact Type updated successfully!");
  ...
  const handlePrintContactType = (data: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("contactTypeData on pdf click", formData);
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/contact-type/create");
        } else {
          navigate("/contact-type/edit/undefined");
        }
      },
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Contact Type saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Contact Type" : "Creating Contact Type"}
        videoSrc={video}
              ...
              {namePerm && (
                <div className="space-y-2">
                  <EditableInput
                    ...
                    tooltipText="Enter contact type"
                    required
                  />
                </div>
              )}
      ...
      {/* Removed employee modal for contact type edit */}
