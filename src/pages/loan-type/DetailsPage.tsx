...
export default function LoanTypeDetails() {
  ...
  // Get loan type data based on selected loan type
  const getLoanTypeData = (loanTypeName: string): DetailsPageTypes => {
    const loanTypeMap: Record<string, DetailsPageTypes> = {
      "Personal Loan": initialData,
      "Home Loan": initialData,
      "Auto Loan": initialData,
      "Business Loan": initialData,
      "Education Loan": initialData,
      "Credit Card": initialData,
    };

    return loanTypeMap[loanTypeName] || initialData;
  };

  const [loanTypeData, setLoanTypeData] = useState<DetailsPageTypes>(
    getLoanTypeData(selectedLoanType)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update loan type data when selection changes
  useEffect(() => {
    setLoanTypeData(getLoanTypeData(selectedLoanType));
  }, [selectedLoanType]);
  ...
  const handlePrintLoanType = (loanTypeData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", loanTypeData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintLoanType(loanTypeData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Loan Type Selection, Loan Type Details */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            {fieldPermissions.loanType && (
              <div className="mt-1">
                <Autocomplete
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((loanTypeData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(loanTypeData.createdAt),
          updated: getRelativeTime(loanTypeData.updatedAt),
          drafted: getRelativeTime(loanTypeData.draftedAt),
          deleted: getRelativeTime(loanTypeData.deletedAt),
        }}
      />
