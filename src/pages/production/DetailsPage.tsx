...
export default function ProductionDetails() {
  ...
  // Get production data based on selected food type
  const getProductionData = (foodType: string): DetailsPageTypes => {
    const productionMap: Record<string, DetailsPageTypes> = {
      "Organic Vegetables": initialData,
      "Fresh Fruits": initialData,
      "Dairy Products": initialData,
      "Meat & Poultry": initialData,
      Seafood: initialData,
      "Bakery Items": initialData,
    };

    return productionMap[foodType] || initialData;
  };

  const [productionData, setProductionData] = useState<DetailsPageTypes>(
    getProductionData(selectedLeaveType)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update production data when selection changes
  useEffect(() => {
    setProductionData(getProductionData(selectedLeaveType));
  }, [selectedLeaveType]);
  ...
  const handlePrintProduction = (productionData: any) => {
    try {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", productionData);
  ...
  return (
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintProduction(productionData);
                }
              }
            : undefined
        }
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((productionData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(productionData.createdAt),
          updated: getRelativeTime(productionData.updatedAt),
          drafted: getRelativeTime(productionData.draftedAt),
          deleted: getRelativeTime(productionData.deletedAt),
        }}
      />
