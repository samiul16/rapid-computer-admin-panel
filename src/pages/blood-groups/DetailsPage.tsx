...
const projectTypeOptions = ["A+", "B+", "AB+", "O+", "A-", "B-", "AB-", "O-"];
...
export default function BloodGroupsDetails() {
  const navigate = useNavigate();
  const { isRTL } = useSelector((state: RootState) => state.language);

  const [keepChanges, setKeepChanges] = useState(false);
  const [isOptionModalOpen, setIsOptionModalOpen] = useState(false);
  const [selectedLeaveType, setSelectedLeaveType] = useState("A+");
  ...
  // get permission
  const canCreate: boolean = usePermission("bloodGroups", "create");
  const canView: boolean = usePermission("bloodGroups", "view");
  const canEdit: boolean = usePermission("bloodGroups", "edit");
  const canDelete: boolean = usePermission("bloodGroups", "delete");
  const canExport: boolean = usePermission("bloodGroups", "export");
  const canPdf: boolean = usePermission("bloodGroups", "pdf");
  const canPrint: boolean = usePermission("bloodGroups", "print");
  const canSeeHistory: boolean = usePermission("bloodGroups", "history");

  // Field-level permissions
  const canViewCode: boolean = usePermission("bloodGroups", "view", "code");
  const canViewBloodGroup: boolean = usePermission(
    "bloodGroups",
    "view",
    "bloodGroup"
  );
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[leavesData]}
          title="Blood Group Details"
          subtitle="Blood Group Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "blood-group-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Blood Group"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/blood-groups")}
        listText="List"
        listPath="blood-groups"
        activePage="view"
        popoverOptions={[
          {
            label: "Create",
            icon: <Plus className="w-5 h-5 text-green-600" />,
            onClick: () => navigate("/blood-groups/create"),
          },
          {
            label: "Edit",
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            {canViewBloodGroup && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">Blood Group</h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.bloodGroup)}
                </div>
              </div>
            )}
