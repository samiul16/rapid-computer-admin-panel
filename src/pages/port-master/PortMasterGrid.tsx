...
export default function PortMasterGrid({
  searchQuery,
  setIsFilterOpen,
  isFilterOpen,
  setIsExportOpen,
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const portCodes = ["QAT", "OMN", "JOR", "LBN", "EGY", "IRQ"];
    const portNames = [
      "Doha Port",
      "Muscat Port",
      "Aqaba Port",
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      portCode: portCodes[Math.floor(Math.random() * portCodes.length)],
      portName: portNames[Math.floor(Math.random() * portNames.length)],
    ...
    if (portDataState.length >= 46) {
      setHasMore(false);
    } else {
      setPortDataState((prev) => [...prev, ...newItems]);
      setPage((prev) => prev + 1);
    }

    setIsLoading(false);
  }, [portDataState.length, isLoading, hasMore]);
  ...
  const handleDeleteClick = (portId: string) => {
    setPortDataState((prevPorts) =>
      prevPorts.map((port) =>
        port.id === portId
          ? {
              ...port,
              isDeleted: port.isDeleted === true ? false : true,
            }
          : port
      )
    );
  ...
  const handleRestoreClick = (portId: string) => {
    setPortDataState((prevPorts) =>
      prevPorts.map((port) =>
        port.id === portId
          ? {
  ...
  // Filter port records based on search query
  const filteredPorts = portDataState.filter(
    (port) =>
      port.portCode.toLowerCase().includes(searchQuery.toLowerCase()) ||
      port.portName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      port.portType.toLowerCase().includes(searchQuery.toLowerCase()) ||
      port.country.toLowerCase().includes(searchQuery.toLowerCase()) ||
      port.contactPerson.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Total {portData.length} port records
        </span>
      </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 items-center gap-2 mb-4">
                  ...
                  <CardTitle
                    ...
                    onClick={() => navigate(`/port-master/1`)}
                  >
                    {port.portName}
                  </CardTitle>
                ...
                {/* Bottom Row - Grid with 3 columns: Port Type | Actions | Country */}
                <div className="grid grid-cols-3 items-center gap-4 pt-2 dark:border-gray-700">
                  {/* Port Type - Left aligned */}
                  <div>
                  ...
                  <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    >
                      <button
                        disabled={port.isDeleted && !canRestore}
                        className={`cursor-pointer p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${
                          port.isDeleted ? "text-blue-500" : "text-red-500"
                        }`}
                        onClick={() => {
                          if (canRestore && port.isDeleted) {
                            handleRestoreClick(port.id);
                            toastRestore("Port record restored successfully");
                          } else {
                            if (canDelete) {
                              handleDeleteClick(port.id);
                              toastDelete("Port record deleted successfully");
                            }
                          }
                        }}
                      >
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">Loading more port records...</span>
              </div>
            </div>
          )}

          {/* End of data indicator */}
          {!hasMore && filteredPorts.length > 12 && (
            <div className="flex justify-center items-center py-8">
              <span className="text-sm text-gray-500 dark:text-gray-400">
                No more port records to load
              </span>
            </div>
          )}
        </div>
        ...
        {isFilterOpen && (
          <div className="w-80 flex-shrink-0 border-l border-gray-200 dark:border-gray-700 pl-4">
            <div className="h-full flex flex-col">
              <GridFilterComponent
                data={portData}
                setFilteredData={setPortDataState}
