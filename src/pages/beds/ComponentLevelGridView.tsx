...
export default function ComponentLevelGridView({
  ...
  const [gridData, setGridData] = useState(bedData);
  const canDelete: boolean = usePermission("beds", "delete");
  const canRestore: boolean = usePermission("beds", "restore");
  const canEdit: boolean = usePermission("beds", "edit");

  // Debug permissions
  console.log("beds Permissions:", {
    canDelete,
    canRestore,
    canEdit,
  });
  ...
  const filteredData = gridData.filter(
    (item) =>
      item.bedName.toLowerCase().includes(searchQuery.toLowerCase()) ||
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (canRestore && item.isDeleted) {
                            ...
                            toastRestore("Bed restored successfully");
                          } else {
                            if (canDelete) {
                              handleDeleteClick(item.id);
                              toastDelete("Bed deleted successfully");
                            }
                          }
                        }}
                      >
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-8">
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">Loading more beds...</span>
              </div>
            </div>
          )}
