...
export default function DoctorDetails() {
  ...
  // Get sub-category data based on selected sub-category
  const getSubCategoryData = (subCategoryName: string): DetailsPageTypes => {
    const subCategoryMap: Record<string, DetailsPageTypes> = {
      Electronics: {
        ...initialData,
        name: "Electronics",
        groups: "Technology",
    ...
    return subCategoryMap[subCategoryName] || initialData;
  };

  const [subCategoryData, setSubCategoryData] = useState<DetailsPageTypes>(
    getSubCategoryData(selectedSubCategory)
  );

  const inputRef = useRef<HTMLInputElement>(null);

  // Update sub-category data when selection changes
  useEffect(() => {
    setSubCategoryData(getSubCategoryData(selectedSubCategory));
  }, [selectedSubCategory]);
    ...
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          ...
          isDefault: "Default Sub-Category",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("pdf click", subCategoryData);
    ...
    <>
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintSubCategory(subCategoryData);
                }
              }
            : undefined
        }
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          {/* Row 1: Sub-Category Selection, Sub-Category Details */}
              ...
              .map(([key, label]) =>
                fieldPermissions[key as keyof DetailsPageTypes] ? (
                  <FieldSection
                    key={key}
                    label={label}
                    value={formatValue((subCategoryData as any)[key])}
                  />
                ) : null
              )}
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(subCategoryData.createdAt),
          updated: getRelativeTime(subCategoryData.updatedAt),
          drafted: getRelativeTime(subCategoryData.draftedAt),
          deleted: getRelativeTime(subCategoryData.deletedAt),
        }}
      />
