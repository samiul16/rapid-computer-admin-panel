...
const initialData: ProjectTypeDataType = {
  ...
  dateAndTime: "06-06-2025 06:00",
  description: "Hola This Someone",
...
export default function WakeUpCallsEditPage({ isEdit = false }: Props) {
  ...
  const canView = usePermission("wakeUpCalls", "view");
  const canEdit = usePermission("wakeUpCalls", "edit");
  const canDelete = usePermission("wakeUpCalls", "delete");
  const canPdf: boolean = usePermission("wakeUpCalls", "pdf");
  const canPrint: boolean = usePermission("wakeUpCalls", "print");
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/wake-up-calls/create");
        } else {
          navigate("/wake-up-calls/edit/undefined");
        }
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      toastSuccess("Wake Up Call updated successfully!");
      handleReset();
    } else {
      toastSuccess("Wake Up Call updated successfully!");
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[formData]}
          title="Wake Up Call Details"
          subtitle="Wake Up Call Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "wake-up-call-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
        ...
        return [
          ...filteredOptions,
          {
            ...
            onClick: () => {
              ...
              toastRestore("Wake Up Call saved as draft successfully");
            },
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? "Editing Wake Up Call" : "Creating Wake Up Call"}
        videoSrc={video}
        videoHeader="Tutorial video"
        listPath="wake-up-calls"
        ...
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-8 relative">
              {permissionsFieldLevel.customerName && (
                <div className="space-y-2">
                  <Autocomplete
                    ref={(el: any) => setRef("customerName")(el)}
                    id="customerName"
                    name="customerName"
              ...
              {permissionsFieldLevel.dateAndTime && (
                <div className="space-y-2">
                  <EditableInput
                    setRef={setRef("dateAndTime")}
                    type="datetime-local"
                    id="dateAndTime"
                    name="dateAndTime"
                    ...
                    onCancel={() =>
                      setFormData({ ...formData, dateAndTime: "" })
                    }
                    labelText="Date and Time"
                    tooltipText="Enter date and time"
                    required
                  />
                </div>
              )}
