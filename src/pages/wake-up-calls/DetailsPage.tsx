...
const projectTypeOptions = ["Customer one", "Customer two", "Customer three"];
...
export default function WakeUpCallsDetails() {
  ...
  // get permission
  // const canCreate: boolean = usePermission("beds", "create");
  // const canView: boolean = usePermission("beds", "view");
  // const canEdit: boolean = usePermission("beds", "edit");
  // const canDelete: boolean = usePermission("beds", "delete");
  // const canExport: boolean = usePermission("beds", "export");
  const canPdf: boolean = usePermission("beds", "pdf");
  const canPrint: boolean = usePermission("beds", "print");
  const canSeeHistory: boolean = usePermission("beds", "history");
  ...
  const getLeavesData = (leaveType: string): TermsData => {
    const leavesMap: Record<string, TermsData> = {
      "Customer one": initialData,
      "Customer two": initialData,
      "Customer three": initialData,
    };
    ...
    try {
      ...
      const blob = await pdf(
        <GenericPDF
          data={[leavesData]}
          title="Wake Up Calls Details"
          subtitle="Wake Up Calls Information"
        />
      ).toBlob();
      ...
      a.href = url;
      a.download = "wake-up-calls-details.pdf";
      a.click();
      console.log("a", a);
      console.log("url", url);
  ...
  return (
    <>
      <PageLayout
        title="Viewing Wake Up Calls"
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/wake-up-calls")}
        listText="List"
        ...
        popoverOptions={[
          {
            ...
            onClick: () => navigate("/wake-up-calls/create"),
          },
          {
            label: "Edit",
      ...
      >
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            {fieldPermissions.dateAndTime && (
              <div className="">
                <h3 className="font-normal mb-1 text-gray-600">
                  Date and Time
                </h3>
                <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
                  {displayValue(leavesData.dateAndTime)}
                </div>
              </div>
            )}
