...
export default function AssetsEditPage({ isEdit = false }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (keepCreating) {
      handleReset();
      focusNextInput("assetName");
    } else {
      navigate("/assets-category");
    }
    toastSuccess("Asset Edited successfully");
  };
  ...
  const handlePrintCountry = (assetsData: any) => {
    try {
      const html = PrintCommonLayout({
        ...
        fieldLabels: {
          code: labels.code,
          assetName: labels.assetNameTooltip,
  ...
  const [popoverOptions, setPopoverOptions] = useState([
    {
      ...
      onClick: () => {
        if (isEdit) {
          navigate("/assets-category/create");
        } else {
          navigate("/assets-category/edit/undefined");
        }
      },
    },
    {
  ...
  return (
    <>
      <PageLayout
        title={isEdit ? labels.editingAsset : labels.creatingAsset}
        videoSrc={video}
