...
export default function SectionGrid({
  ...
  const canRestore: boolean = usePermission("drivers", "restore");
  ...
  const loadMoreData = useCallback(async () => {
    ...
    const newItems = Array.from({ length: ITEMS_PER_PAGE }, (_, index) => ({
      id: `${Date.now()}-${index}`,
      employee: `Drivers ${Date.now()}-${index}`,
      vehicle: "Toyota Corolla",
      status: "active",
      isDeleted: false,
    }));
  ...
  const filteredRoomSize = RoomSizeData.filter((roomSize) =>
    roomSize.employee.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Total {filteredRoomSize.length} Drivers
        </span>
      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (canRestore && roomSizes.isDeleted) {
                            ...
                            toastRestore("Drivers restored successfully");
                          } else {
                            if (canDelete) {
                              handleDeleteClick(roomSizes.id);
                              toastDelete("Drivers deleted successfully");
                            }
                          }
                        }}
                      >
