...
export default function InsurancesDetailsPage() {
  ...
  const canCreate: boolean = usePermission("insurances", "create");
  const canView: boolean = usePermission("insurances", "view");
  const canPrint: boolean = usePermission("insurances", "print");
  const canSeeHistory: boolean = usePermission("insurances", "history");
  ...
  let InsurancesData = {
    name: "Life Insurance",
    description: "Regular Insurance Description",
  ...
  const handlePrintInsurances = (InsurancesData: any) => {
    try {
  ...
  const handleSwitchChange = (checked: boolean) => {
    ...
    // if (checked) {
    //   setTimeout(() => handlePrintInsurances(InsurancesData), 100);
    // }
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("InsurancesData on pdf click", InsurancesData);
  ...
  return (
    <>
      <PageLayout
        title={t("button.viewingInsurances")}
        videoSrc={video}
        videoHeader="Tutorial video"
        onListClick={() => navigate("/insurances")}
        listText="List"
        listPath="insurances"
        activePage="view"
        popoverOptions={[
        ...
        onExport={
          canPdf && canPrint
            ? () => {
                ...
                if (printEnabled) {
                  handlePrintInsurances(InsurancesData);
                }
              }
            : undefined
        }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
          <div className="mt-1">
            <Autocomplete
              ...
              value={InsurancesData.employeeName}
          ...
          <div className="">
            ...
            <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
              {displayValue(InsurancesData.iqamaNo)}
            </div>
          </div>

          <div className="">
            <h3 className="font-normal mb-1 text-gray-600">Insurance</h3>
            <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
              {displayValue(InsurancesData.name)}
            </div>
          </div>
          ...
          <div className="">
            ...
            <div className="w-full py-1 text-gray-900 font-bold text-md dark:text-white">
              {displayValue(InsurancesData.description)}
            </div>
          </div>
      ...
      <HistoryDataTable
        ...
        title="History"
        statusInfo={{
          created: getRelativeTime(InsurancesData.createdAt),
          updated: getRelativeTime(InsurancesData.updatedAt),
          drafted: getRelativeTime(InsurancesData.draftedAt),
          deleted: getRelativeTime(InsurancesData.deletedAt),
        }}
      />
