...
const initialData: TableAssignType = {
...
export default function TableAssignsCreatePage({ isEdit = false }: Props) {
  ...
  const tableNoInputRef = useRef<EditableInputRef>(null);
  const waiterNameInputRef = useRef<EditableInputRef>(null);
  ...
  // Form state
  const [formData, setFormData] = useState<TableAssignType>({
    waiterName: "",
    tableNo: "",
    description: "",
    photo: "",
  ...
  const focusNextInput = (currentField: string) => {
    ...
    switch (currentField) {
      case "tableNo":
        waiterNameInputRef.current?.focus();
        break;
      case "waiterName":
  ...
  const handleExportPDF = async () => {
    ...
    try {
      console.log("tableAssignData on pdf click", formData);
  ...
  return (
    <>
      <PageLayout
        title={
          isEdit ? t("form.editingTableAssign") : t("form.creatingTableAssign")
        }
        videoSrc={video}
        ...
        popoverOptions={[
          {
            ...
            onClick: () => {
              ...
              if (isEdit) {
                ...
                navigate("/table-assigns/create");
              } else {
                // Navigate to edit page
                navigate("/table-assigns/edit/undefined");
              }
            },
          },
          {
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            <div className="md:col-span-3 space-y-2">
              <h3 className="font-medium mb-1">{t("form.tableNo")}</h3>
              <EditableInput
                ref={tableNoInputRef}
                id="tableNo"
                name="tableNo"
                ...
                onNext={() => focusNextInput("tableNo")}
                onCancel={() => {}}
                tooltipText="Please enter table no"
                required
              />
            </div>

            <div className="md:col-span-3 space-y-2">
              <h3 className="font-medium mb-1">{t("form.waiterName")}</h3>
              <EditableInput
                ref={waiterNameInputRef}
                id="waiterName"
                name="waiterName"
                className="w-full h-10"
                value={formData.waiterName}
                onChange={handleChange}
                onNext={() => focusNextInput("waiterName")}
                onCancel={() => {}}
                tooltipText="Please enter waiter name"
                required
              />
            </div>
